		...
		Если мПодменюВременныеТаблицыКнопки.ИспользоватьМенеджерВременныхТаблиц.Пометка Тогда 
			...
			Если гИнициализацияVBScript() тогда
				...
				Для каждого ВременнаяТаблица Из мВременныеТаблицы Цикл
					...
					гСоответствиеВременныеТаблицы.Вставить(ИмяТаблицы, гВременнаяТаблицаВТЗ(ИмяТаблицы, лЗапрос.МенеджерВременныхТаблиц));
				КонецЦикла; 
...
Процедура СохранитьЗапросыВОблакоДействие(Режим, СохранитьТекущийЗапрос = Ложь)
	...
	ЗаписьJSON = ПолучитьЗапросыВJSON(ДеревоЗапросов, ПараметрыЗапросов, ИсполняемыйКод, Длг.Имя, лЛокально, лВыгружатьТолькоИзмененные, ТекущийЗапрос, лПолучитьРекурсивно);
