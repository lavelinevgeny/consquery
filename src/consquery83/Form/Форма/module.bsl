...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	МассивМенюДляДобавления.Добавить(Новый Структура("Имя, Контейнер, Картинка, Индекс", "СформироватьКод1С"    , ЭлементыФормы.КоманднаяПанельФормы     , ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Индекс(ЭлементыФормы.КоманднаяПанельФормы.Кнопки.Найти("ПолучитьИзЗапроса")) + 1));
...
Процедура КнопкаМенюСервисВыполнитьКод(Кнопка)
	ВыполнитьКод();
КонецПроцедуры // КнопкаМенюСервисВыполнитьКод()
...
Процедура КнопкаМенюСервисПечатьДанныхТаблицы(Кнопка)
	...
	Если ТекСтраница.Имя = "Результат" Тогда 
		СформироватьПечатнуюФормуКоллекции(мЭлементФормыРеквизитаТаблицы.Значение);
	Иначе
		
		Если ТекСтраница <> Неопределено Тогда
			Если Найти(ТекСтраница.Имя, "СтраницаСРезультатом_") > 0 Тогда
				СформироватьПечатнуюФормуКоллекции(ЭлементыФормы[СтрЗаменить(ТекСтраница.Имя, "СтраницаСРезультатом_", "ТаблицаСРезультатом_")].Значение);
			Конецесли;
		Конецесли;		
	КонецЕсли;
...
Процедура КнопкаМенюСервисПоискСсылок(Кнопка)
	...
	Если ЗначениеЗаполнено(лТаблицаНайденныхСсылок) Тогда 
		СформироватьПечатнуюФормуКоллекции(лТаблицаНайденныхссылок);		
	КонецЕсли;	
...
Процедура КодЗапросСформироватьКод(Кнопка)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		СкопироватьТекстВБуфер(ПолучитьФорму("Точка"),Результат);
...
Процедура СкопироватьТекстВБуфер(Форма, Текст) Экспорт
...
Конецпроцедуры // СкопироватьТекстВБуфер()
...
Процедура ПровестиДокументыДействие(ПровестиДокумент)
	...
	лДокументыТипВсеСсылки = Документы.ТипВсеСсылки;
	...
	Для Каждого СтрокаСРезультатомЗапроса Из лРезультатЗапроса Цикл		
		...
		Если НЕ лДокументыТипВсеСсылки.СодержитТип(ТипЗнч(СтрокаСРезультатомЗапроса[лИмяКолонки])) Тогда
			лОшибки = лОшибки + 1;
			ВывестиСообщение("Ошибка в строке " + Инд + ": значение '" + СтрокаСРезультатомЗапроса[лИмяКолонки] + "' не является документом.");
		Иначе
...
Процедура СформироватьКодДляОтладки()
	...
	Если Вопрос("Скопировать в буфер [Да] / Вывести код с инструкцией [Нет]?", РежимДиалогаВопрос.ДаНет,,, "Код для отладки запроса") = КодВозвратаДиалога.Да Тогда
		СкопироватьТекстВБуфер(ПолучитьФорму("Точка"), "ВнешниеОбработки.Создать(""" + ТекИмяФайла + """).гОткрытьФормуДляОтладки(ЗАПРОС)");
	Иначе
		ТД = Новый ТекстовыйДокумент(); 		
