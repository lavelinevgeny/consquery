...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущаяСтрокаПриОткрытии      = ?(ЗначениеЗаполнено(Параметры.ТекущаяСтрокаПриОткрытии), Параметры.ТекущаяСтрокаПриОткрытии, 0);
	...
	Элементы.ИсполняемыйКодСписок.ТекущаяСтрока = 0;
	
	лКореньМеню = ЗаполнитьМенюПоДереву(КоманднаяПанель, ОбъектОбработки(ТипОбработка).гПолучитьМеню("ПредустановленныйКод", Истина, Ложь));
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТекущаяСтрокаПриОткрытии <> 0 Тогда 
		Элементы.ИсполняемыйКодСписок.ТекущаяСтрока = ИсполняемыйКодСписок.НайтиСтроки(Новый Структура("Идентификатор", ТекущаяСтрокаПриОткрытии))[0].ПолучитьИдентификатор();
	КонецЕсли;
...
Процедура ПодставитьПредустановленныйКод(ВидПредустановленногоКода)
	...
	ИначеЕсли ВидПредустановленногоКода = ТипыПредустановленногоКода(ТипОбработка).РедактироватьРегистрСведений Тогда 
		...
		лСписокОбъектовМетаданных  = ПолучитьгСписокОбъектовМетаданныхИзТекстаНаСервере(ТекстТекущегоЗапроса, "РегистрыСведений", лЗначениеПоУмолчанию);		
	...
	ИначеЕсли ВидПредустановленногоКода = ТипыПредустановленногоКода(ТипОбработка).РедактироватьДокументы Тогда 
		лЗначениеПоУмолчанию       = Неопределено;
		лСписокОбъектовМетаданных  = ПолучитьгСписокОбъектовМетаданныхИзТекстаНаСервере(ТекстТекущегоЗапроса, "Документы", лЗначениеПоУмолчанию);		
...
Процедура ВставитьТекстВПолеФормы(РеквизитФормы, ПолеТекстовогоДокумента, ТекстДляДобавления)
	...
	ПолеТекстовогоДокумента.УстановитьГраницыВыделения(СтрокаНач,КолонкаНач,СтрокаКон + лИзменяемыйТекст.КоличествоСтрок(),СтрДлина(лИзменяемыйТекст.ПолучитьСтроку(лИзменяемыйТекст.КоличествоСтрок())));

КонецПроцедуры // ВставитьТекстВПолеФормы()

#КонецОбласти

#Область ВызовГлобальныхпрофедурИФункций

Функция ПолучитьгСписокОбъектовМетаданныхИзТекстаНаСервере(ИсходныйТекст, ФильтрМетаданных, ЗначениеПоУмолчанию)
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьСписокОбъектовМетаданныхИзТекста(ИсходныйТекст, ФильтрМетаданных, ЗначениеПоУмолчанию);
КонецФункции
