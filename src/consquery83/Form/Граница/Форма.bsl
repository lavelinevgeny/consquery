...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//#Рефаторинг. нет смысла передавать коллекцию ДоступныеТипыДанных.. можно ограничиться только именем типа и самим типом. 
	Если НЕ Параметры.Свойство("ДоступныеТипыДанных", ДоступныеТипыДанных) Тогда		
		ПоказатьСообщениеПользователю("Для формы ""Граница"" не передан параметр ""ДоступныеТипыДанных"".");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(лГраница) = Тип("Граница") Тогда
		...
		Тип                 = ОбъектОбработки().гСформироватьСписокТипов(ДоступныеТипыДанных, Неопределено).НайтиПоЗначению(ИмяТипа).Представление;
