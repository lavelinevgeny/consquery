...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	лПунктМеню = Новый Структура();
	лПунктМеню.Вставить("Имя"             , "СформироватьКод1С");
	лПунктМеню.Вставить("Контейнер"       , Элементы.МенюОперации);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОбновитьИмяПользователяВОблаке" Тогда 
		Если Параметр.ИмяПользователя <> ИмяПользователяВОблаке Тогда 
			Если Не ЗначениеЗаполнено(Параметр.ИмяПользователя) Тогда  ИмяФайлаСЗапросом = ""; КонецЕсли;
			ИмяПользователяВОблаке = Параметр.ИмяПользователя;
			ОбновитьЗаголовок();
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СохраненныеПечатныеФормыНаСервере(ТипОбработка, СеансовыеДанные, КаталогВременныхФайлов, ИмяСервера)
	
	КаталогВременныхФайлов = КаталогВременныхФайлов();
	ИмяСервера             = ИмяКомпьютера();
	
	лМаскаПоиска   = гСтроковыеКонтанты(ТипОбработка, СеансовыеДанные, "ПрефиксФайлаСПечатнойФормой") + "*";
...
КонецФункции // СохраненныеПечатныеФормыНаСервере()
...
Процедура ЗагрузитьСохраненныеПечатныеФормы(Команда)
	...
	СписокПечатныхФорм = СохраненныеПечатныеФормыНаСервере(ТипОбработка, СеансовыеДанные, лКаталогВременныхФайлов, лИмяСервера);
...
КонецПроцедуры // КнопкаМенюСервисВыгрузитьКолонкуРезультатаЗапросаВПараметр()
...
Процедура ДобавитьЗапросИзОблакаПродолжение(Длг, ДополнительныеПараметры) Экспорт
	...
	Если Длг = Неопределено ИЛИ Длг = КодВозвратаДиалога.Отмена Или Не Длг.Выбран Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаПропуститьПервуюСтрокуТаблицы(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Таймаут ИЛИ Результат = КодВозвратаДиалога.Отмена Тогда 
		Возврат;
	Иначе
		Если Результат = КодВозвратаДиалога.Да Тогда 
			Параметры.Данные.Вставить("ПропуститьПервуюСтроку", Истина);
		КонецЕсли;
		
		ПараметрыВопроса = Новый Структура();
		ПараметрыВопроса.Вставить("Данные"          , Параметры.Данные);
		ПараметрыВопроса.Вставить("ВыбраннаяКолонка", Параметры.ВыбранныйЭлемент.Значение);
		
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаПытатьсяОпределятьТипЗначений", ЭтаФорма, ПараметрыВопроса), 
...
Процедура ЗавершениеВыборКолонкиВыгрузитьВПараметр(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		Если ДополнительныеПараметры.Действие = "СоздатьПараметрИзБуфера" Тогда 
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаПропуститьПервуюСтрокуТаблицы", ЭтаФорма, ПараметрыВопроса), 
				"Пропустить первую строку таблицы?", 
				РежимДиалогаВопрос.ДаНетОтмена,, 
				КодВозвратаДиалога.Нет);
			
		ИначеЕсли ДополнительныеПараметры.Действие = "ВыгрузитьТаблицуРезультатаВПараметр" Тогда 
			ОбработкаДействияВыгрузитьВПараметрНаСервере(?(ВыбранныйЭлемент.Значение = Константы().ВсеКолонки, Неопределено, ВыбранныйЭлемент.Значение), "ИзТаблицыРезультата");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗакрытияВопросаПытатьсяОпределятьТипЗначений(Результат, Параметры) Экспорт
	...
	Иначе
		...
		Если Результат = КодВозвратаДиалога.Да Тогда
			...
			лАнализируемаяСтрока = ?(лКоличествоСтрок > 1, 1, 0);
...
Процедура ОбработкаДействияСоздатьПараметрИзБуфера()
	...
	ПараметрыВыбораКолонки = Новый Структура;
	ПараметрыВыбораКолонки.Вставить("Действие", "СоздатьПараметрИзБуфера");
	ПараметрыВыбораКолонки.Вставить("Данные"  , лДанныеБуфера);
	
	лСписокКолонок.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ЗавершениеВыборКолонкиВыгрузитьВПараметр", ЭтаФорма, ПараметрыВыбораКолонки),
		"Выберите колонку для загрузки в таблицу значений");
