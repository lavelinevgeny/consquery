...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокТипов           = ОбработкаОбъект.гСформироватьСписокТипов(СеансовыеДанные, СтруктураКЭШ);
...
Процедура ЗагрузитьСохраненныеПечатныеФормы(Команда)
	...
	СписокПечатныхФорм = гСписокФайловНаСервереПоПрефиксу(ТипОбработка, лКаталогВременныхФайлов, лИмяСервера, "ПрефиксФайлаСПечатнойФормой");
...
Процедура ВыполнитьПослеПолученияФайлов(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ПолученныйФайл Из ПолученныеФайлы Цикл
		...
		Если лКаталогСФайлами = Неопределено Тогда 
			лКаталогСФайлами = лФайл.Путь;
		КонецЕсли;
	...
	ЗапуститьПриложение(лКаталогСФайлами);
...
Процедура ОткрытьФормуСТаблицей(ИмяТаблицы, АдресСоответствияТаблиц)
	...
	ПараметрыФормы.Вставить("Заголовок"           , "Просмотр таблицы <" + ИмяТаблицы + ">"); // #рефакторинг переделать на формирование строки по шаблону как в БСП
...
Процедура ПоказатьВыделеннуюТаблицу(ЭтоВременнаяТаблица)
	...
	Если ПолучитьТаблицыЗапросаНаСервере(ТекстТекущегоЗапроса, ЭтоВременнаяТаблица).Найти(лВыделеннаяТаблица)= Неопределено Тогда 
...
Процедура ПерейтиКОписаниюТаблицы(ЭтоВременнаяТаблица);
	...
	ЭлементСТекстомТекущегоЗапроса().УстановитьГраницыВыделения(лПозицияТаблицыВТекстеЗапроса, лПозицияТаблицыВТекстеЗапроса);	
...
Процедура ЗавершениеВыбораТипаПараметра(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		СвойстваПараметра = Новый Структура("Значение, Картинка, Представление");
		ЗаполнитьЗначенияСвойств(СвойстваПараметра, ВыбранныйЭлемент);
		СвойстваПараметра.Вставить("Значение", ВыбранныйЭлемент.Значение);
		
		ИнициализацияТипаИЗначенияПараметра(Элементы.ПараметрыТекущегоЗапроса.ТекущаяСтрока, СвойстваПараметра);
		Модифицированность = Истина;
	КонецЕсли;
...
#Область Процедуры_и_функции_режима_отладки
...
Процедура ЗагрузитьФайлыНаКлиент(Операция)
	...
	СписокСхемСКД = гСписокФайловНаСервереПоПрефиксу(ТипОбработка, лКаталогВременныхФайлов, лИмяСервера, лПрефикс);
...
&НаСервереБезКонтекста
Функция гТипЗначенияВСтроку(ТипЗначения, СеансовыеДанные, СтруктураКЭШ, ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гТипЗначенияВСтроку(ТипЗначения, СеансовыеДанные, СтруктураКЭШ);
КонецФункции
...
Функция ЗаполнитьПараметрыНаФормеИПолучитьТекстЗапросаИзКода1сНаСервере(ТипОбработка, Код1С, ПростаяОбработкаТекста)
	
	лЗапрос          = гПолучитьЗапросИзТекста(ТипОбработка, Код1С, ПростаяОбработкаТекста);
	лОбъектОбработки = ОбъектОбработки(ТипОбработка);
	
	// #Заполнениепараметровзапроса 1
	Для каждого лПараметр Из лЗапрос.Параметры Цикл
		Попытка
			лСтрокаСПараметром = лОбъектОбработки.гНайтиЗначениеБезУчетаРегистра(лПараметр.Ключ, ПараметрыТекущегоЗапроса, "Имя");
		...
		Исключение
			Сообщить("Неудачная попытка установки параметра " + лПараметр.Ключ + "[" + лПараметр.Значение + "](ошибка:" + ОписаниеОшибки() + ")");
		КонецПопытки; 
	КонецЦикла; 
...
&НаКлиенте
Функция ЭлементСТекстомТекущегоЗапроса()
	Возврат Элементы.ТекстТекущегоЗапроса;
КонецФункции // ЭлементСТекстомТекущегоЗапроса()

&НаКлиенте
Функция ВыделенныйТекстТекущегоЗапроса()
	Возврат ЭлементСТекстомТекущегоЗапроса().ВыделенныйТекст;
КонецФункции // ВыделенныйТекстТекущегоЗапроса()

&НаКлиенте
Функция ПолучитьУстановитьТекущийТекстЗапроса(НовыйТекстЗапроса = Неопределено) //#рефакторинг уйти от данной функции
	
	лЭлементФормы = Элементы.ТекстТекущегоЗапроса;
	
	Если НовыйТекстЗапроса = Неопределено Тогда 
		Возврат Новый Структура("ЭлементФормы, ТекстЗапроса, ВыделенныйТекст", лЭлементФормы, ТекстТекущегоЗапроса, лЭлементФормы.ВыделенныйТекст);
	КонецЕсли;
...
Функция ЗаполнитьПараметрыЗапросов(ТекстЗапроса, ТекстОшибки)
	...
	Для Каждого Параметр Из лПараметры Цикл
		...
		лСтрокаСПараметром = лОбъектОбработки.гНайтиЗначениеБезУчетаРегистра(лИмяПараметра, ПараметрыТекущегоЗапроса, "Имя");
		Если лСтрокаСПараметром = Неопределено Тогда 
			лСтрокаСПараметром                     = ПараметрыТекущегоЗапроса.Добавить();
...
Процедура ОбработкаДействийЗаполнитьПараметрыИзЗапроса()
	...
	Если Не ПустаяСтрока(ТекстТекущегоЗапроса) Тогда
		...
		Если Не ЗаполнитьПараметрыЗапросов(ТекстТекущегоЗапроса, ТекстОшибки) Тогда 
...
Процедура ИнициализацияТипаИЗначенияПараметра(ИдентификаторСтрокиПараметра, СвойстваПараметра, НеИнициироватьЗначение = Ложь)
	...
	ТипСтрока = гТипЗначенияВСтроку(ТекущийТипЗначение, СеансовыеДанные, СтруктураКЭШ, ТипОбработка);
	
	Если ТекущийТипЗначение = "ТаблицаЗначений"
		Или ТекущийТипЗначение = "МоментВремени" 
		Или ТекущийТипЗначение = "Массив" 
...
&НаКлиенте
Функция СтрокаСЗапросом(ИдентификаторСтроки = Неопределено)
...
&НаКлиенте
Функция ИдентификаторЗапроса(ИдентификаторСтроки = Неопределено)
...
&НаКлиенте
Функция РодительСтрокиСЗапросом(ИдентификаторСтроки)
...
&НаКлиенте
Функция ПараметрЗапроса(ИдентификаторСтроки)
	
	Возврат СтрокаСЗапросом(ИдентификаторСтроки).ПолучитьРодителя();
			
КонецФункции // ПараметрЗапроса()
	...
	Иначе
		...
		Для Сч = 0 По лДанныеБуфера.КоличествоКолонок - 1 Цикл
			...
			Если лТекущееИмяКолонки = "" Тогда
				лТекущееИмяКолонки = лОбъектОбработки.гПолучитьСледующееУникальноеИмя("Поле", лМассивКолонок);
			КонецЕсли;
...
Процедура ОбработкаДействияПолучениеЗапросаИзКода1СПолучитьЗапрос(ПростаяОбработкаТекста)
	...
	ПолучитьУстановитьТекущийТекстЗапроса(ЗаполнитьПараметрыНаФормеИПолучитьТекстЗапросаИзКода1сНаСервере(ТипОбработка, лСтрокаБуфера, ПростаяОбработкаТекста));
...
Процедура ВыполнитьЗапросСервер(ТекстЗапроса, ИдентификаторЗапроса, ТекстСообщения, Ошибка, ПараметрыПодключенияКВнешнейБазе = Неопределено) Экспорт
	...
	Если ПараметрыПодключенияКВнешнейБазе = Неопределено Тогда 
		Если Не РезультатЗапросаВДерево Тогда 
			лРезультатЗапроса = лРезультат.Выгрузить();
		Иначе
			лРезультатЗапроса = лРезультат.Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
		КонецЕсли;
	Иначе
	...
	Если ПоказыватьВременныеТаблицы Тогда 
		...
		Для Каждого лТекущаяТаблица Из лВременныеТаблицы Цикл
			лСоответствиеВременныеТаблицы.Вставить(лТекущаяТаблица, ОбъектОбработки(ТипОбработка).гВременнаяТаблицаВТЗ(лТекущаяТаблица, Запрос.МенеджерВременныхТаблиц));
		КонецЦикла;
	...
	Если ПоказыватьЗапросыПакета Тогда 
		...
		Для Каждого лТекущаяТаблица Из лТаблицыПакета Цикл
			Если Сч = лРезультатПакета.ВГраница() Тогда 
				Прервать;
			КонецЕсли;
			лСоответствиеТаблицПакета.Вставить(лТекущаяТаблица, лРезультатПакета[Сч].Выгрузить());
			Сч = Сч + 1;
		КонецЦикла;
