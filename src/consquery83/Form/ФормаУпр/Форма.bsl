...
Процедура КомандаОткрытьНастройкиОбработки(Команда)
	ОткрытьФорму("ВнешняяОбработка.КонсольЗапросов.Форма.ФормаУпрНастройки", 
		Новый Структура("СохранитьПередВыполнением, РезультатЗапросаВДерево, ПоказыватьВременныеТаблицы", 
			СохранитьПередВыполнением, РезультатЗапросаВДерево, ПоказыватьВременныеТаблицы), Вычислить(ЭтотОбъектФорма));
КонецПроцедуры
...
&НаКлиенте
Процедура СообщитьОбОшибкеПоказаВременнойТаблицы(ИмяВременнойТаблицы)
	Сообщить("Временная таблица <" + ИмяВременнойТаблицы +"> не найдена в запросе. Возможно изменился запрос.
	|Выполните запрос еще раз и попробуйте снова.", СтатусСообщения.Важное);
КонецПроцедуры // СообщитьОбОшибкеПоказаВременнойТаблицы()

&НаСервереБезКонтекста
Функция гСвойстваПунктаМенюПоказатьВременнуюТаблицу(ТипОбработка, СеансовыеДанные)
	Возврат ОбъектОбработки(ТипОбработка, СеансовыеДанные).гСвойстваПунктаМенюПоказатьВременнуюТаблицу()
КонецФункции

&НаКлиенте
Процедура ОткрытьФормуСВременнойТаблицей(ИмяВременнойТаблицы)
	
	лАдресТаблицыЗначений = ПоложитьВременнуюТаблицуНаСервер(ИмяВременнойТаблицы, АдресСоответствияВременныхТаблиц);
	
	Если Не ЗначениеЗаполнено(лАдресТаблицыЗначений) Тогда 
		СообщитьОбОшибкеПоказаВременнойТаблицы(ИмяВременнойТаблицы);
		Возврат;
	КонецЕсли;
	
	лРежимОткрытияФормы  = гРежимыОткрытияФормы(ТипОбработка, СеансовыеДанные).ПоказатьТаблицу;
	...
	ПараметрыФормы.Вставить("Заголовок"           , "Просмотр временной таблицы <" + ИмяВременнойТаблицы + ">"); // #рефакторинг переделать на формирование строки по шаблону как в БСП
	
	ОткрытьФорму(СтрЗаменить(ИмяФормы, ".ФормаУпр", "") + ".ФормаУпрПустая", ПараметрыФормы, ЭтаФорма);			
	
КонецПроцедуры // ОткрытьФормуСВременнойТаблицей()
	
&НаСервере
Функция ПоложитьВременнуюТаблицуНаСервер(ИмяВременнойТаблицы, АдресСоответствияВременныхТаблиц)
	
	лСоответствияВременныхТаблиц = ПолучитьИзВременногоХранилища(АдресСоответствияВременныхТаблиц);
	
	лВременнаяТаблица = лСоответствияВременныхТаблиц[ИмяВременнойТаблицы];
...
КонецФункции // ПоложитьВременнуюТаблицуНаСервер()

Процедура ПерерисоватьСпискиВременныхТаблицНаСервере(ВременныеТаблицы)
	
	лСвойстваПунктаМенюПоказатьВременнуюТаблицу = ОбъектОбработки(ТипОбработка, СеансовыеДанные).гСвойстваПунктаМенюПоказатьВременнуюТаблицу();
	
	лИмяЭлементаРодителя = лСвойстваПунктаМенюПоказатьВременнуюТаблицу.ИмяКорня;
	лРодитель            = Элементы[лИмяЭлементаРодителя];
	лДействие            = лСвойстваПунктаМенюПоказатьВременнуюТаблицу.Действие;
	...
	Для Каждого ТекВременнаяТаблица из ВременныеТаблицы Цикл 
		
		лИмяПунктаВременнойТаблицы = лИмяЭлементаРодителя + ТекВременнаяТаблица;
		
		КомандаФормы = Команды.Добавить(лИмяПунктаВременнойТаблицы);
		КомандаФормы.Действие = лДействие; 
		
		Кнопка = Элементы.Добавить(лИмяПунктаВременнойТаблицы, Тип("КнопкаФормы"), лРодитель);
		Кнопка.ИмяКоманды = лИмяПунктаВременнойТаблицы;
		Кнопка.Заголовок  = ТекВременнаяТаблица;
	КонецЦикла;
	
КонецПроцедуры // ПерерисоватьСпискиВременныхТаблицНаСервере()

&НаКлиенте
Процедура ПерерисоватьСпискиВременныхТаблиц()
	
	Если Не ПоказыватьВременныеТаблицы Тогда 
		Элементы.ПодменюВременныеТаблицы.Видимость = Ложь;
	Иначе
		
		лВременныеТаблицы = ПолучитьВременныеТаблицыНаСервере(ТекстТекущегоЗапроса);
		
		лПоказыватьВременныеТаблицы = (лВременныеТаблицы.Количество() > 0);
		
		Элементы.ПодменюВременныеТаблицы.Видимость = лПоказыватьВременныеТаблицы;
		
		Если лПоказыватьВременныеТаблицы Тогда 
			ПерерисоватьСпискиВременныхТаблицНаСервере(лВременныеТаблицы);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьВременныеТаблицыНаСервере(ТекстЗапроса)
	...
	лВременныеТаблицы = Новый Массив;
	...
	Если Не Ошибка Тогда 
		...
		Для Каждого ТекущийЗапрос ИЗ СхемаЗапроса.ПакетЗапросов Цикл 
			Если ТекущийЗапрос.ТаблицаДляПомещения = "" Тогда 
				Продолжить;
			КонецЕсли;
			лВременныеТаблицы.Добавить(ТекущийЗапрос.ТаблицаДляПомещения);
		КонецЦикла;
	КонецЕсли;
	
	Возврат лВременныеТаблицы;
	
КонецФункции // ПолучитьВременныеТаблицыНаСервере()
...
&НаКлиенте
Процедура ЗагрузитьФайлыНаКлиент(Команда)
	
	Перем лКаталогВременныхФайлов, лИмяСервера;
	
	Если Команда = "ЗагрузитьСохраненныеСхемыСКД" Тогда 
	...
	ИначеЕсли Команда = "ЗагрузитьСериализованныеОбъекты" Тогда
	...
	СписокСхемСКД = СписокФайловНаСервереПоПрефиксу(ТипОбработка, СеансовыеДанные, лКаталогВременныхФайлов, лИмяСервера, лПрефикс);
...
//////////////////////////////////////////////////
// Меню ПодменюВременныеТаблицы

&НаКлиенте
Процедура КнопкаМенюПодменюВременныеТаблицыПоказатьВременнуюТаблицу(Команда)
	ОткрытьФормуСВременнойТаблицей(СтрЗаменить(Команда.Имя, гСвойстваПунктаМенюПоказатьВременнуюТаблицу(ТипОбработка, СеансовыеДанные).ИмяКорня, ""));
...
Процедура КнопкаМенюПодменюВременныеТаблицыПоказатьВыделеннуюВременнуюТаблицу(Команда)
	...
	лВыделеннаяВТ = СокрЛП(ВыделенныйТекстТекущегоЗапроса());
	
	Если ПолучитьВременныеТаблицыНаСервере(ТекстТекущегоЗапроса).Найти(лВыделеннаяВТ)= Неопределено Тогда 
		СообщитьОбОшибкеПоказаВременнойТаблицы(лВыделеннаяВТ);
		Возврат;
	КонецЕсли;
	
	ОткрытьФормуСВременнойТаблицей(лВыделеннаяВТ);
...
Процедура КнопкаМенюПодменюВременныеТаблицыПерейтиКОписаниюВременнойТаблицы(Команда)
	...
	лПозицияВременнойТаблицыВТекстеЗапроса = Найти(ВРег(ТекстТекущегоЗапроса), "ПОМЕСТИТЬ " + ВРег(лВыделеннаяВТ));
	
	Если лПозицияВременнойТаблицыВТекстеЗапроса = 0 Тогда 
		Сообщить("В тексте запроса не найдена строка ""Поместить " + лВыделеннаяВТ + """", СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
	
	ЭлементСТекстомТекущегоЗапроса().УстановитьГраницыВыделения(лПозицияВременнойТаблицыВТекстеЗапроса, лПозицияВременнойТаблицыВТекстеЗапроса);	
...
Процедура ВыполнитьЗапросИзФормыПродолжение(Параметры)
	...
	Если Параметры.Свойство("Операция", лОперация) Тогда 
		Если лОперация = мОперацииСЗапросами.Выполнить Тогда 
			ЗапуститьВыполнениеЗапроса();
		ИначеЕсли лОперация = мОперацииСЗапросами.ВыполнитьУдаленно Тогда 
			ЗапуститьВыполнениеЗапроса(Истина);
		Иначе
...
Функция ФункционалДоступен()
	...
	КонецЕсли;
	Возврат Истина;
КонецФункции // ФункционалДоступен()
...
Процедура ОбработкаДействияПолучениеКода1СПоЗапросуПолучитьКод1С(Режим)
	...
	ПараметрыВопроса.Вставить("Строка"   , гСформироватьКодЗапросаДля1С(ТипОбработка, СеансовыеДанные, ТекстТекущегоЗапроса, ПараметрыВСоответствие(), Режим));
	ПоказатьВопрос(Новый ОписаниеОповещения("ВопросЗавершение", ЭтаФорма, ПараметрыВопроса), ТекстВопроса, КнопкиВопроса, 10, КодВозвратаДиалога.Нет, "Вопрос", КодВозвратаДиалога.Нет);
...
Процедура ВыполнитьЗапросСервер(ТекстЗапроса, ИдентификаторЗапроса, ТекстСообщения, Ошибка, ПараметрыПодключенияКВнешнейБазе = Неопределено) Экспорт
	...
	// Сохраняем временные таблицы запроса
	Если ПоказыватьВременныеТаблицы Тогда 

		лСоответствиеВременныеТаблицы = Новый Соответствие;
		
		лВременныеТаблицы = ПолучитьВременныеТаблицыНаСервере(Запрос.Текст);
		
		Для Каждого лТекущаяТаблица Из лВременныеТаблицы Цикл
			лСоответствиеВременныеТаблицы.Вставить(лТекущаяТаблица, ОбъектОбработки(ТипОбработка, СеансовыеДанные).гВременнаяТаблицаВТЗ(лТекущаяТаблица, Запрос.МенеджерВременныхТаблиц));
		КонецЦикла;
