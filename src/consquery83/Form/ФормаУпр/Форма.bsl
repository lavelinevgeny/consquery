...
Функция ДобавитьПараметрыТекущегоЗапросаПоТекстуЗапроса(ТекстЗапроса, ТекстОшибки)
	...
	Для Каждого Параметр Из лПараметрыЗапроса Цикл
		...
		Если Параметр.ТипЗначения.Типы().Количество() > 0 Тогда 
			...
			КонецЕсли;
			//ИнициализацияТипаИЗначенияПараметраПоИдентиифкатору(лСтрокаСПараметром.СистемныйИдентификаторСтроки, ЭлементСпискаВСтруктуру(ДанныеПоТипу));
			ИнициализацияТипаИЗначенияПараметра(лСтрокаСПараметром, ЭлементСпискаВСтруктуру(ДанныеПоТипу), Ложь);
			
		Иначе
			//ИнициализацияТипаИЗначенияПараметраПоИдентиифкатору(лСтрокаСПараметром.СистемныйИдентификаторСтроки, Новый Структура("Значение, Представление, Картинка", ТипЗнч(""), "Строка", Неопределено));
