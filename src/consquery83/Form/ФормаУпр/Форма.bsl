...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокТипов           = ОбработкаОбъект.гСформироватьСписокТипов(ОбработкаОбъект.гЗначениеИзСеансовыхДанных("ДоступныеТипыДанных", СеансовыеДанные), СтруктураКЭШ);
	...
	лПутькКартинке = гЗначениеИзСеансовыхДанных("ПутькКартинкам", СеансовыеДанные, ТипОбработка);
...
Процедура ПриОткрытии(Отказ)
	...
	мОперацииСЗапросами   = гОперацииСЗапросами(ТипОбработка);
	мТипыИсточниковДанных = гТипыИсточниковДанных(ТипОбработка);
	...
	ПараметрыФормыДлительнойОперации = ПараметрыОжидания(Неопределено); // #Рефакторинг: при передаче формы - ошибка "Отсутствует отображение для типа 'ФормаКлиентскогоПриложения'"
	...
	ИдентификаторСессии = гВосстановитьИдентификаторСессииConsqueryCloud(ТипОбработка);
...
Процедура РезультатЗапросаПолучитьТипЗначения(Команда)
	...
	Если лТекущиеДанные <> Неопределено Тогда 
		...
		ПоказатьВводСтроки(Новый ОписаниеОповещения("ПоказатьВводСтрокиЗавершение", ЭтаФорма), гПредставлениеТипа(ТипОбработка, ТипЗнч(СодержимоеЯчейки)), "Тип значения", ,Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПараметрыЗапросовЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	лПараметрыформы.Вставить("ДоступныеТипыДанных", ПоместитьВоВременноеХранилище(гЗначениеИзСеансовыхДанных("ДоступныеТипыДанных", СеансовыеДанные, ТипОбработка)));
...
Процедура ПараметрыТекущегоЗапросаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	Если Элемент.ТекущиеДанные.Тип = "" Тогда 
		...
		ИнициализацияТипаИЗначенияПараметра(Элементы.ПараметрыТекущегоЗапроса.ТекущаяСтрока, ЭлементСпискаВСтруктуру(лВыбранныйЭлемент));
	КонецЕсли;
...
Процедура СкопироватьОбработкуДляОтладки(Команда)
	...
	Попытка
		...
		ПутьКФайлуСОбработкойНаСервере = СкопироватьОбработкуДляОтладкиНаСервере(ТипОбработка, Адрес, ПутьКФайлуСОбработкой);	
	Исключение
	...
	Если ПутьКФайлуСОбработкойНаСервере <> Неопределено Тогда 
		...
		ТД.УстановитьТекст(ПомощьПоОтладке(ТипОбработка, ПутьКФайлуСОбработкойНаСервере));
...
Процедура ЗагрузитьЗапросыДляОтладки(Команда)
	...
	Если лРезультатЗагрузки = 0 Тогда 
		ПоказатьПредупреждение(Новый ОписаниеОповещения("ПоказатьПредупреждениеЗавершение", ЭтотОбъектФорма), "Не найдено данных для отладки.", 10, "Информация");
	Иначе
...
Процедура ЗагрузитьСохраненныеПечатныеФормы(Команда)
	...
	СписокПечатныхФорм = гСписокФайловНаСервереПоПрефиксу(ТипОбработка, КаталогВременныхФайлов, ИмяСервера, "ПрефиксФайлаСПечатнойФормой");
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ВыполнитьПослеПолученияФайлов", ЭтотОбъектФорма, Новый Структура("КаталогВременныхФайлов", КаталогВременныхФайлов));
...
&НаСервереБезКонтекста
Функция гМетодыПоискаВСтроке(ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гМетодыПоискаВСтроке();
КонецФункции
...
&НаСервереБезКонтекста
Функция гРежимыЗакрытияФормыВводаПроизвольногоКода(ТипОбработка, Режим)
	Возврат ОбъектОбработки(ТипОбработка).гРежимыЗакрытияФормыВводаПроизвольногоКода()[Режим];
КонецФункции
...
&НаСервереБезКонтекста
Функция гРежимыОткрытияФормы(ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гРежимыОткрытияФормы();
КонецФункции

&НаСервереБезКонтекста
Функция гЗаголовокОбработки(ТипОбработка, ИмяПользователяВОблаке, ИмяФайлаСЗапросом)

	Возврат ОбъектОбработки(ТипОбработка).гЗаголовокОбработки(ИмяПользователяВОблаке, ИмяФайлаСЗапросом);

КонецФункции // гЗаголовокОбработки()

&НаСервереБезКонтекста
Функция гНазваниеОбработки(ТипОбработка, ВернутьПолноеНазвание = Истина)

	Возврат ОбъектОбработки(ТипОбработка).гНазваниеОбработки(ВернутьПолноеНазвание);
...
&НаСервереБезКонтекста
Функция гПрефиксСпецПоляНаСервере(ТипОбработка)
	
	Возврат ОбъектОбработки(ТипОбработка).гПрефиксСпецПоля()
...
Функция гПрефиксСпецПоля()
	...
	Если Не мКэшЗначений.Свойство(КлючКэшаЗначений) Тогда 
		мКэшЗначений.Вставить(КлючКэшаЗначений, гПрефиксСпецПоляНаСервере(ТипОбработка));
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция гРазложитьСтрокуВМассивПодстрок(ТипОбработка, Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено, СокращатьНепечатаемыеСимволы = Ложь)
	Возврат ОбъектОбработки(ТипОбработка).гРазложитьСтрокуВМассивПодстрок(Строка, Разделитель, ПропускатьПустыеСтроки, СокращатьНепечатаемыеСимволы);
КонецФункции // гРазложитьСтрокуВМассивПодстрок()

#КонецОбласти

#Область ФункцииСОбъектомЗапрос

// Возвращает массив запроса по тексту запроса
// Параметры:
...
&НаСервереБезКонтекста
Функция гПолучитьСледующееУникальноеИмя(ТипОбработка, Знач ИсходноеИмя, Коллекция, ИмяПоля = Неопределено) Экспорт
	
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьСледующееУникальноеИмя(ИсходноеИмя, Коллекция, ИмяПоля);
...
// #Рефакторинг: гТипЗначенияВСтроку vs гПредставлениеТипа
&НаСервереБезКонтекста
Функция гТипЗначенияВСтроку(ТипЗначения, ДоступныеТипыДанных, СтруктураКЭШ, ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гТипЗначенияВСтроку(ТипЗначения, ДоступныеТипыДанных, СтруктураКЭШ);
КонецФункции

&НаСервереБезКонтекста
Функция гПредставлениеТипа(ТипОбработка, Тип)
	Возврат ОбъектОбработки(ТипОбработка).гПредставлениеТипа(Тип);
КонецФункции
...
&НаСервере
Функция гНайтиЗначениеБезУчетаРегистраВПараметрахТекЗапросаНаСервере(ТипОбработка, Значение)
	
	Коллекция = ТаблицаЗначенийВМассивСтруктур(ПараметрыТекущегоЗапроса.Выгрузить());
	
	Возврат ОбъектОбработки(ТипОбработка).гНайтиЗначениеБезУчетаРегистра(Значение, Коллекция, "Имя", гМетодыПоискаВСтроке(ТипОбработка).Вначале, Истина);	
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция гНайтиЗначениеБезУчетаРегистра(ТипОбработка, Значение, Знач Коллекция, Поле = Неопределено, МетодПоиска = Неопределено, МножественныйВыбор = Ложь)
	
	Возврат ОбъектОбработки(ТипОбработка).гНайтиЗначениеБезУчетаРегистра(Значение, Коллекция, Поле, МетодПоиска, МножественныйВыбор);	
...
Процедура ОбновитьЭлементыФормыДляТекущегоЗапроса(идСтрокиСЗапросом)
	...
	Если идСтрокиСЗапросом <> Неопределено Тогда 
		...
		лИдентификаторЗапроса = лСтрокаСЗапросом.Идентификатор;
		
	Иначе
		лИдентификаторЗапроса = Неопределено;
	КонецЕсли;
	...
	ДобавитьПараметрыТекущегоЗапросаИзОбщейТаблицыПараметров("", ЕстьПараметрыБезТипов); // добавляем глобальные параметры
	
	Если НЕ лИдентификаторЗапроса = Неопределено Тогда 
		// добавляем параметры конкретного запроса
		ДобавитьПараметрыТекущегоЗапросаИзОбщейТаблицыПараметров(лИдентификаторЗапроса, ЕстьПараметрыБезТипов); 
	КонецЕсли;
...
Процедура ОперацииВставитьЗапросИзКода1С(ПростаяОбработкаТекста)
	...
	лЗапрос = гПолучитьЗапросИзТекста(ТипОбработка, лСтрокаБуфера, ПростаяОбработкаТекста);
...
Процедура ОперацииПолучитьКод1СПоЗапросу(Режим)
	...
	ПараметрыВопроса.Вставить("Строка"   , гСформироватьКодЗапросаДля1С(ТипОбработка, ТекстТекущегоЗапроса, ПараметрыВСоответствие(), Режим));
...
Процедура ОбработатьОперациюПослеСохраненияЗапросов(Операция, ДополнительныеПараметры)
	...
	ИначеЕсли Операция = "ЗакрытьФорму" Тогда // #рефакторинг определить константу
...
&НаСервереБезКонтекста
Функция ПомощьПоОтладке(ТипОбработка, ПутьКФайлуСОбработкойНаСервере)
	Возврат ОбъектОбработки(ТипОбработка).ПомощьПоОтладке(ПутьКФайлуСОбработкойНаСервере);
КонецФункции // гСтроковыеКонстанты()

&НаСервереБезКонтекста
Функция СкопироватьОбработкуДляОтладкиНаСервере(ТипОбработка, Адрес, ПутьКФайлуСОбработкой)
	Возврат ОбъектОбработки(ТипОбработка).СкопироватьОбработкуДляОтладкиНаСервере(Адрес, ПутьКФайлуСОбработкой);
КонецФункции // гСтроковыеКонстанты()

Функция ЗагрузитьЗапросыДляОтладкиНаСервере()
	
	лОбъектОбработки              = ОбъектОбработки(ТипОбработка);
...
&НаСервереБезКонтекста
Функция гЗначениеИзСеансовыхДанных(Имя, СеансовыеДанные, ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гЗначениеИзСеансовыхДанных(Имя, СеансовыеДанные)
КонецФункции
...
&НаСервереБезКонтекста
Функция гСписокФайловНаСервереПоПрефиксу(ТипОбработка, КаталогВременныхФайлов, ИмяСервера, Префикс)
	
	Возврат ОбъектОбработки(ТипОбработка).гСписокФайловНаСервереПоПрефиксу(КаталогВременныхФайлов, ИмяСервера, Префикс);
...
Процедура ЗагрузитьФайлыНаКлиент(Операция)
	...
	СписокФайловНаСервере = гСписокФайловНаСервереПоПрефиксу(ТипОбработка, КаталогВременныхФайлов, ИмяСервера, лПрефикс);
...
// #Рефакторинг: унифицировать процедуру добавления параметров запроса
...
Функция ДобавитьПараметрыТекущегоЗапроса(ПараметрыЗапроса)
	...
	Для Каждого лПараметр Из ПараметрыЗапроса Цикл
		...
		Попытка
			...
			лСтрокаСПараметром = гНайтиЗначениеБезУчетаРегистра(ТипОбработка, лПараметр.Ключ, ПараметрыТекущегоЗапроса, "Имя");
...
Функция ДобавитьПараметрыТекущегоЗапросаПоТекстуЗапроса(ТекстЗапроса, ТекстОшибки)
	...
	Для Каждого Параметр Из лПараметрыЗапроса Цикл
		...
		лСтрокаСПараметром = гНайтиЗначениеБезУчетаРегистра(ТипОбработка, лИмяПараметра, ПараметрыТекущегоЗапроса, "Имя");
		...
		Если Параметр.ТипЗначения.Типы().Количество() > 0 Тогда 
			...
			КонецЕсли;
			ИнициализацияТипаИЗначенияПараметра(лСтрокаСПараметром.ПолучитьИдентификатор(), ЭлементСпискаВСтруктуру(ДанныеПоТипу));
		Иначе
...
&НаСервере
Процедура ИнициализацияТипаИЗначенияПараметра(ИдентификаторСтрокиПараметра, СвойстваТипаПараметра, НеИнициироватьЗначение = Ложь)
	...
	ТипСтрока = гТипЗначенияВСтроку(
		ТекущийТипЗначение, 
		ОбъектОбработки(ТипОбработка).гЗначениеИзСеансовыхДанных("ДоступныеТипыДанных", СеансовыеДанные), 
		СтруктураКЭШ, 
		ТипОбработка);
	...
	Если Не НеИнициироватьЗначение Тогда 
		Если ТекущийТипЗначение = "ТаблицаЗначений" Или ТекущийТипЗначение = "МоментВремени" 
			Или ТекущийТипЗначение = "Массив" Или ТекущийТипЗначение = "Граница" Тогда 
...
// Возвращает информаию по типу для переданного значения
// Параметры:
...
Процедура ЗаполнитьПустыеТипыУПараметровТекущегоЗапроса()
	...
	Для каждого лПараметр Из ПараметрыТекущегоЗапроса Цикл
		...
		Если ПустаяСтрока(лПараметр.Тип) И лПараметр.Значение <> "" Тогда 
			...
			ИнициализацияТипаИЗначенияПараметра(лПараметр.ПолучитьИдентификатор(), лДанныеПоТипуДляТекущегоЗначения, Истина);
		КонецЕсли;
	КонецЦикла; 	
...
Процедура ОткрытьФормуСТаблицей(ИмяТаблицы, АдресСоответствияТаблиц)
	...
	лРежимОткрытияФормы  = гРежимыОткрытияФормы(ТипОбработка).ПоказатьТаблицу;
...
Процедура ПерерисоватьПодменюНаСервере(СписокМеню, ИмяМетодаПолученияСвойствМеню)
	...
	лСвойстваПунктаМеню  = Вычислить("ОбъектОбработки(ТипОбработка)." + ИмяМетодаПолученияСвойствМеню + "()");
...
&НаСервереБезКонтекста
Функция гСвойстваПунктаМенюПоказатьВременнуюТаблицу(ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гСвойстваПунктаМенюПоказатьВременнуюТаблицу()
КонецФункции

&НаСервереБезКонтекста
Функция гСвойстваПунктаМенюПоказатьТаблицуПакета(ТипОбработка)
	Возврат ОбъектОбработки(ТипОбработка).гСвойстваПунктаМенюПоказатьТаблицуПакета()
КонецФункции
...
&НаКлиенте
Процедура СпроситьОчиститьРезультатЗапросаЕслиМногоДанных(Удаленно)
	
	ПараметрыВопроса = Новый Структура();
	ПараметрыВопроса.Вставить("Удаленно", Удаленно);
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СпроситьОчиститьРезультатЗапросаЕслиМногоДанныхЗавершение", ЭтаФорма, ПараметрыВопроса), "Таблица с результатом содержит много данных.
		|Очистить ее перед выполнение нового запроса(это сильно ускорит его выполнение)? ", 
...
Процедура СохранитьЗапросыВФайлПродолжение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено Тогда 
		...
		ЗаписьJSON = ПолучитьЗапросыВJSON(ДеревоЗапросов, ПараметрыЗапросов, ИсполняемыйКод, лПолноеИмяФайла, лЛокально, лВыгружатьТолькоИзмененные, лТекущийЗапрос, лПолучитьРекурсивно);
...
Процедура СохранитьЗапросыВОблакоПродолжение(Длг, ДополнительныеПараметры) Экспорт
	...
	ЗаписьJSON = ПолучитьЗапросыВJSON(ДеревоЗапросов, ПараметрыЗапросов, ИсполняемыйКод, Длг.Имя, лЛокально, лВыгружатьТолькоИзмененные, лТекущийЗапрос, лПолучитьРекурсивно);
	
	лДанные = ЗаписьJSON.Закрыть();
	
	Если ЭтоСохранениеПакета Тогда 
		лРезультат  = гСохранитьФайлСЗапросомВОблаке(ТипОбработка, лИдентификаторСессии, Длг.ИдПакета, Длг.Имя, истина, лДанные); // #рефакторинг добавить использование настройки режима обновления всех записей или только измененных
	Иначе
		лРезультат  = гСохранитьЗапросыВОблаке(ТипОбработка, лИдентификаторСессии, Длг.ИдПакета, Длг.ИдРодительскогоЗапроса, Длг.ИдЗапроса, Длг.Имя, Длг.ВключатьПодчиненныеЗапросы, лДанные);
	КонецЕсли;
...
Процедура ОбработкаВыполненияКода(Результат, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(Результат) = Тип("Структура") Тогда 
		...
		Если Результат.Режим = гРежимыЗакрытияФормыВводаПроизвольногоКода(ТипОбработка, "Сохранить") ИЛИ 
			Результат.Режим = гРежимыЗакрытияФормыВводаПроизвольногоКода(ТипОбработка, "Выполнить") Тогда 
		...
		Если Результат.Режим = гРежимыЗакрытияФормыВводаПроизвольногоКода(ТипОбработка, "Выполнить") Тогда 
...
Процедура ЗавершениеВыбораТипаПараметра(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ИнициализацияТипаИЗначенияПараметра(Элементы.ПараметрыТекущегоЗапроса.ТекущаяСтрока, ЭлементСпискаВСтруктуру(ВыбранныйЭлемент));
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ПроверитьКоличествоСтрокВРезультатеИЗапуститьВыполнениеЗапроса(Удаленно = Ложь)
	...
	Иначе 
		СпроситьОчиститьРезультатЗапросаЕслиМногоДанных(Удаленно)
	КонецЕсли;
...
Процедура МенюИнформация(Команда)
	...
	ПоказатьВыборИзМеню(Оповещение, ПолучитьМенюНаСервереСписком("Информация", ТипОбработка, лПутькКартинке));
...
Процедура МенюДополнительно(Команда)
	...
	лПутькКартинке = гЗначениеИзСеансовыхДанных("ПутькКартинкам", СеансовыеДанные, ТипОбработка); //#рефакторинг реализовать получение картнки в виде метода
	
	Оповещение = Новый ОписаниеОповещения("ВыборМенюДействие", Вычислить(ЭтотОбъектФорма)); 
	ПоказатьВыборИзМеню(Оповещение, ПолучитьМенюНаСервереСписком("Дополнительно", ТипОбработка, лПутькКартинке));	
...
Функция СписокОбъектовМетаданныхИзТекстаСервере(ТекстЗапроса)
	...
	// возврат ОбъектОбработки(ТипОбработка).гСписокОбъектовМетаданныхИзТекста(ТекстЗапроса);
...
Процедура КнопкаМенюПодменюВременныеТаблицыПоказатьВременнуюТаблицу(Команда)
	ОткрытьФормуСТаблицей(СтрЗаменить(Команда.Имя, гСвойстваПунктаМенюПоказатьВременнуюТаблицу(ТипОбработка).ИмяКорня, ""), АдресСоответствияВременныхТаблиц);
...
Процедура КнопкаМенюПодменюТаблицыПакетаПоказатьВременнуюТаблицу(Команда)
	ОткрытьФормуСТаблицей(СтрЗаменить(Команда.Имя, гСвойстваПунктаМенюПоказатьТаблицуПакета(ТипОбработка).ИмяКорня, ""), АдресСоответствияТаблицПакета);
...
Функция ПродолжитьФормироватьКПНаСервере(Режим, ВыбранноеЗначение)
	Возврат ОбъектОбработки(ТипОбработка).гПродолжитьФормироватьКП(Режим, ВыбранноеЗначение)
КонецФункции
...
Функция ПолучитьДанныеДляПодбораКПНаСервере(ТекстЗапроса, лПозицияКурсораВТексте)
	...
	Результат = ОбъектОбработки(ТипОбработка).гПолучитьДанныеДляПодбораКП(ТекстЗапроса, лПозицияКурсораВТексте, 
...
&НаСервереБезКонтекста
Процедура гВывестиОшибкуВыполненияКода(ТипОбработка, ТекстОшибки, КодДляВыполнения)
	ОбъектОбработки(ТипОбработка).гВывестиОшибкуВыполненияКода(ТекстОшибки, КодДляВыполнения);
КонецПроцедуры
...
Процедура РасшифроватьСпецПоле(ИмяПоля, ИдентификаторСтроки, ТипПоля)
	...
	лРежимОткрытияФормы = гРежимыОткрытияФормы(ТипОбработка).Расшифровка;
...
Процедура ОформитьНажатиеКнопки(ИмяРеквизитаКнопки, Нажата) // #рефакторинг проверить необходимость данной процедуры
	...
	//	Элементы.ДекорацияПоказатьДеревоЗапросов.Картинка = ПолучитьКартинкуНаСервере("МенюСкрыть", ТипОбработка);
	// иначе
	//	Элементы[ИмяРеквизитаКнопки].ЦветРамки = мЦветСтиляЦветРамки;
	//	Элементы.ДекорацияПоказатьДеревоЗапросов.Картинка = ПолучитьКартинкуНаСервере("МенюПоказать", ТипОбработка);
	// конецЕсли;
...
Процедура СоздатьРеквизитCРезультатомЗапроса(АдресРезультата)
	...
	лОбработкаОбъект  = ОбъектОбработки(ТипОбработка);
...
Процедура ОбновитьЗаголовок()
	...
	ЭтаФорма.Заголовок = гЗаголовокОбработки(ТипОбработка, ИмяПользователяВОблаке, ИмяФайлаСЗапросом)
...
&НаСервереБезКонтекста
Функция ПолучитьМенюНаСервереСписком(ИмяМеню, ТипОбработка, ПутькКартинке)
	
	лМенюСписком = ОбъектОбработки(ТипОбработка).гПолучитьМеню(ИмяМеню, Истина, Истина, ПутькКартинке);
...
&НаСервереБезКонтекста
Функция гСформироватьКодЗапросаДля1С(ТипОбработка, ТекстЗапроса, ПараметрыЗапроса, Режим)
	Возврат ОбъектОбработки(ТипОбработка).гСформироватьКодЗапросаДля1С(ТекстЗапроса, ПараметрыЗапроса, Режим, Истина);
КонецФункции

&НаСервереБезКонтекста
Функция гПолучитьЗапросИзТекста(ТипОбработка, Код1С, ПростаяОбработкаТекста)
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьЗапросИзТекста(Код1С, ПростаяОбработкаТекста);
КонецФункции
...
Процедура ЗагрузитьСтаруюВерсиюДереваРекурсивноНаСервере(Знач Источник, Знач НовоеДерево = Неопределено, Знач СпецСтрока = Неопределено)
	...
	Если НовоеДерево = Неопределено Тогда 
		...
		СпецСтрока  = ОбъектОбработки(ТипОбработка).гПараметрыСпецСтрокиДереваЗапросов().Значение;
...
Функция ПолучитьЗапросыВJSON(ДеревоЗапросов, ПараметрыЗапросов, ИсполняемыйКод, ИмяЗапроса, Локально, ВыгружатьТолькоИзмененные, Знач СтрокаСЗапросом, ПолучитьРекурсивно)
	...
	Данные.Вставить("header"    , Новый Структура("version, date", гНазваниеОбработки(ТипОбработка), ТекущаяДата()));
...
&НаКлиенте
Процедура ДобавитьПараметрыИзСтруктурыJSON(parameters, ЗаменяемыеИД = Неопределено, ИдентификаторыПараметров = Неопределено)
	...
	Для каждого лПараметр Из parameters Цикл
		...
		Если ИдентификаторыПараметров <> Неопределено Тогда 
			ИдентификаторыПараметров.Добавить(лНоваяСтрока.ИдентификаторСтроки);
		КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьКодИзСтруктурыJSON(codes, ЗаменяемыеИД = Неопределено, ИдентификаторыКодовЗапроса = Неопределено)
	...
	Для каждого лСтрокаСкодом Из codes Цикл
		...
		Если ИдентификаторыКодовЗапроса <> Неопределено Тогда 
			ИдентификаторыКодовЗапроса.Добавить(лНоваяСтрока.ИдентификаторСтроки);
		КонецЕсли;
...
Функция ДобавитьВеткуДереваЗапросов(Данные, Родитель)
	...
	ИдентификаторыПараметров   = Новый Массив;
	ИдентификаторыКодовЗапроса = Новый Массив;
	ЗаменяемыеИД               = Новый Соответствие;
	
	// читаем запросы
	лНоваяВетка = ПрочитатьЗапросыИзФайлаJSONРекурсивно(Данные.querys, Родитель, ЗаменяемыеИД);
	
	ДобавитьПараметрыИзСтруктурыJSON(Данные.parameters, ЗаменяемыеИД, ИдентификаторыПараметров);
	ДобавитьКодИзСтруктурыJSON(Данные.codes, ЗаменяемыеИД, ИдентификаторыКодовЗапроса);
	...
	Если ИдентификаторыПараметров.Количество() = 0 Тогда 
		ИдентификаторПараметра = Неопределено
	Иначе
		ИдентификаторПараметра = ИдентификаторыПараметров[ИдентификаторыПараметров.ВГраница()]
	КонецЕсли;
	
	Если ИдентификаторыКодовЗапроса.Количество() = 0 Тогда 
		ИдентификаторКода = Неопределено
	Иначе
		ИдентификаторКода = ИдентификаторыКодовЗапроса[ИдентификаторыКодовЗапроса.ВГраница()]
	КонецЕсли;
...
Процедура ВыполнитьДействияПриЗакрытииФормы()
	...
	лПутьККартинкам = гЗначениеИзСеансовыхДанных("ПутькКартинкам", СеансовыеДанные, ТипОбработка);
...
//&НаСервереБезКонтекста
// функция ПолучитьКартинкуНаСервере(ИмяКартинки, ТипОбработка)
//	Возврат ОбъектОбработки(ТипОбработка).ПолучитьКартинку(ИмяКартинки);
...
Функция гНажатиеНаКнопкуВыборМенюИнформацияНаСервере(ИмяКнопки, ТолькоВернутьКоманду)
	Возврат ОбъектОбработки(ТипОбработка).гНажатиеНаКнопкуВыборМенюИнформация(ИмяКнопки, ТолькоВернутьКоманду);
КонецФункции
...
Функция ТекстВТаблицуНаСервере(МассивКолонокСтрокиТаблицы, Текст, ДобавлятьКолонки, Разделитель, ЗагружатьВТаблицуРезультата = Истина)
	...
	Для сч = 1 По Текст.КоличествоСтрок() Цикл
		лМассивКолонокСтрокиТаблицы = гРазложитьСтрокуВМассивПодстрок(ТипОбработка, Текст.ПолучитьСтроку(сч), Разделитель);
...
&НаСервереБезКонтекста
Функция гСохранитьФайлСЗапросомВОблаке(ТипОбработка, соединение, идВОблаке, имя, обновлятьВсеЗаписи, данные, НастройкиПрокси = Неопределено);
	Возврат ОбъектОбработки(ТипОбработка).гСохранитьФайлСЗапросомВОблаке(соединение, идВОблаке, имя, обновлятьВсеЗаписи, данные, НастройкиПрокси);
КонецФункции // гСохранитьФайлСЗапросомВОблаке()

&НаСервереБезКонтекста
Функция гСохранитьЗапросыВОблаке(ТипОбработка, соединение, идПакета, идРодительскогоЗапроса, ИдЗапроса, имя, включатьПодчиненныеЗапросы, данные, НастройкиПрокси = Неопределено);
	Возврат ОбъектОбработки(ТипОбработка).гСохранитьЗапросыВОблаке(соединение, идПакета, идРодительскогоЗапроса, ИдЗапроса, имя, включатьПодчиненныеЗапросы, данные, НастройкиПрокси);
...
Функция ЗагрузитьЗапросыИзОблака(идПакетаВОблаке, идЗапроса, идСтрокиКода, включатьПодчиненные, Родитель)
	...
	лРезультат = гПолучитьЗапросыИзОблака(ТипОбработка, ИдентификаторСессии, идПакетаВОблаке, идЗапроса, идСтрокиКода, включатьПодчиненные);
...
&НаСервереБезКонтекста
Функция гПолучитьЗапросыИзОблака(ТипОбработка, ИдентификаторСессии, идПакетаВОблаке, идЗапроса, идСтрокиКода, включатьПодчиненные);
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьЗапросыИзОблака(ИдентификаторСессии, идПакетаВОблаке, идЗапроса, идСтрокиКода, включатьПодчиненные);
КонецФункции // гПолучитьЗапросыИзОблака()

&НаСервереБезКонтекста
Функция гПолучитьИмяПодключенногоПользователяВОблаке(ТипОбработка, ИдентификаторСессии)
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьИмяПодключенногоПользователяВОблаке(ИдентификаторСессии);
...
Процедура УстановитьИмяПользователяВОблаке()
	...
	Если ЗначениеЗаполнено(ИдентификаторСессии) Тогда 
		лРезультат = гПолучитьИмяПодключенногоПользователяВОблаке(ТипОбработка, ИдентификаторСессии);
		Если лРезультат.Статус = "OK" Тогда 
...
&НаСервереБезКонтекста
Функция гПреобразоватьВПравильноеНазваниеНаСервере(ТипОбработка, ИсходноеНазвание)
	Возврат ОбъектОбработки(ТипОбработка).гПреобразоватьВПравильноеНазвание(ИсходноеНазвание);
...
Функция ПодготовитьДанныеИзБуфера()
	...
	Иначе
		...
		лМассивКолонокИзБуфера = гРазложитьСтрокуВМассивПодстрок(ТипОбработка, лТекст.ПолучитьСтроку(1), лДанныеБуфера.Разделитель);
		...
		Для Сч = 0 По лДанныеБуфера.КоличествоКолонок - 1 Цикл
			лТекущееИмяКолонки = гПреобразоватьВПравильноеНазваниеНаСервере(ТипОбработка, лМассивКолонокИзБуфера[Сч]);
			Если лТекущееИмяКолонки = "" Тогда
				//#рефакторинг неоптимальный переход с клиента на сервер 
				лТекущееИмяКолонки = гПолучитьСледующееУникальноеИмя(ТипОбработка, "Поле", лМассивКолонок);
			КонецЕсли;
...
Функция ВыполнитьНаСервере(КодДляВыполнения)
	...
	Исключение
		лТекстОшибки = ОписаниеОшибки();
		гВывестиОшибкуВыполненияКода(ТипОбработка, лТекстОшибки, КодДляВыполнения);
		Возврат Ложь;
	КонецПопытки;
КонецФункции
...
Функция ПолучитьКоординатыОшибкиНаСервере(ТекстОшибки)
	Возврат ОбъектОбработки(ТипОбработка).гПолучитьКоординатыОшибки(ТекстОшибки);
КонецФункции
...
Процедура ЗапуститьВыполнениеЗапроса(Удаленно = Ложь)
	...
	Если Не Ошибка Тогда 
		...
		Если Удаленно Тогда 
			...
			лПараметрыСоединенияСИБ = гНайтиЗначениеБезУчетаРегистраВПараметрахТекЗапросаНаСервере(ТипОбработка, 
		...
		Если ПараметрыПодключенияКВнешнейБазе = Неопределено Тогда 
			...
			Если ПоказыватьВременныеТаблицы Тогда 
				...
				Для Каждого лТекущаяТаблица Из лВременныеТаблицы Цикл
					лСоответствиеВременныеТаблицы.Вставить(лТекущаяТаблица.Представление, ОбъектОбработки(ТипОбработка).гВременнаяТаблицаВТЗ(лТекущаяТаблица, Запрос.МенеджерВременныхТаблиц));
				КонецЦикла;
			...
			Если ПоказыватьЗапросыПакета Тогда 
				...
				Для Каждого лТекущаяТаблица Из лТаблицыПакета Цикл
					//Если Сч = лРезультатПакета.ВГраница() Тогда 
					//	Прервать;
					//КонецЕсли;
					лСоответствиеТаблицПакета.Вставить(лТекущаяТаблица.Представление, лРезультатПакета[Сч].Выгрузить());
					Сч = Сч + 1;
				КонецЦикла;
