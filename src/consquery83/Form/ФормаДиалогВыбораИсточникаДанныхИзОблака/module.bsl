...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	мИндексыКартинкиДляТипов = гПолучитьИндексыКартинкиДляТипов();
	...
	Если Не Отказ Тогда 
		...
		Если Не Отказ Тогда 
			ЭлементыФормы.НеВключатьПодчиненныеЗапросы.Видимость = (типЭлемента = типыИсточниковДанных().Запрос И гЭтоЗапись(Режим));
		КонецЕсли;		
	КонецЕсли;
КонецПроцедуры
...
Процедура КоманднаяПанельКонтМенюСоздатьНовыйПакет(Кнопка)
	ПоказатьВводСтроки(Новый ОписаниеОповещения("ПоказатьВводСтрокиСоздатьНовыйПакетПродолжить", ЭтаФорма, Новый Структура),
		гСтроковыеКонтанты("ИмяНовогоПакетаЗапроса"), "Введите название пакета", 100, Истина);
КонецПроцедуры
