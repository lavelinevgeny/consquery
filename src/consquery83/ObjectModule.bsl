...
// Формирует список всех возможных типов обработки.
//
// Параметры:
//	СписокДобавленныхТипов - возвращает в данный параметр список типов, добавленных "вручную".
//
Функция гСформироватьСписокТипов(СтруктураКЭШ = Неопределено, СписокДобавленныхТипов = Неопределено) Экспорт
	
	Если ДоступныеТипыДанных = Неопределено Тогда 
		ДоступныеТипыДанных = Метаданные().Реквизиты.ДоступныеТипыДанных.Тип;
	КонецЕсли;
	...
	КартинкаТипа = БиблиотекаКартинок.Ресурс; // #Рефакторинг подобрать подходящие картинки для типов
	
	СписокТипов = Новый СписокЗначений;
	СписокТипов.Добавить("Строка"                , НСтр("en='String';ru='Строка'")                ,, КартинкаТипа);
	СписокТипов.Добавить("Число"                 , НСтр("en='Number';ru='Число'")                 ,, КартинкаТипа);
	СписокТипов.Добавить("Дата"                  , НСтр("en='Date';ru='Дата'")                    ,, КартинкаТипа);
	СписокТипов.Добавить("Булево"                , НСтр("en='Boolean';ru='Булево'")               ,, КартинкаТипа);
	СписокТипов.Добавить("Граница"               , НСтр("en='Border';ru='Граница'")               ,, КартинкаТипа);
	СписокТипов.Добавить("МоментВремени"         , НСтр("en='Point in time';ru='Момент времени'") ,, КартинкаТипа);
	СписокТипов.Добавить("СписокЗначений"        , НСтр("en='Value list';ru='Список значений'")   ,, КартинкаТипа);
	СписокТипов.Добавить("ТаблицаЗначений"       , НСтр("en='Values table';ru='Таблица значений'"),, КартинкаТипа);
	СписокТипов.Добавить("Массив"                , НСтр("en='Array';ru = 'Массив'")               ,, КартинкаТипа);
	СписокТипов.Добавить("ВидДвиженияНакопления" , НСтр("en='AccumulationRecordType';ru = 'Вид движения накопления'"),, 
		КартинкаТипа);
	СписокТипов.Добавить("ВидДвиженияБухгалтерии", НСтр("en='AccountingRecordType';ru = 'Вид Движения бухгалтерии'") ,, 
		КартинкаТипа);
	...
	Для каждого Стр Из НеПримитивныеТипы Цикл
		...
		ИмяМетаданногоДляТипа = ПолучитьИмяБазовогоТипаПоТипуОбъекта(Стр.Значение, СтруктураКЭШ);
		КартинкаДляТипа       = ПолучитьКартинкуПоМетаданному(ИмяМетаданногоДляТипа, СтруктураКЭШ);
		
		СписокТипов.Добавить(ЗначениеТипа, ПредставлениеТипа,, КартинкаДляТипа);
	КонецЦикла;
...
////
//Функция гТипЗначенияВСтроку(ТипЗначения, ДоступныеТипыДанных, СтруктураКЭШ) Экспорт
//	
//	//#рефакторинг - передавать сразу списоктипов, чтобы не получать несколько раз
//	
//	СписокДобавленныхТипов = Новый СписокЗначений;
//	
//	СписокТипов = гСформироватьСписокТипов(ДоступныеТипыДанных, СтруктураКЭШ, СписокДобавленныхТипов);
//	
