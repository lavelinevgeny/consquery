...
Функция СформироватьПечатнуюФормуКоллекции(Коллекция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если НЕ (лТипКоллекции = Тип("РезультатЗапроса") ИЛИ лТипКоллекции = Тип("ТаблицаЗначений") ИЛИ лТипКоллекции = Тип("ДеревоЗначений")) Тогда 
		лТекстОшибки = "Операция не доступна для типа %ТипКоллекции%. Доступные типы: {ТаблицаЗначений|ДеревоЗначений|РезультатЗапроса}.";
	...
	ИначеЕсли ТипЗнч(Коллекция) = Тип("ТаблицаЗначений") ИЛИ 
		ТипЗнч(Коллекция) = Тип("ДеревоЗначений") Тогда 
		ИсточникДанных = Коллекция.Скопировать();
	КонецЕсли;
	
	Если ТипЗнч(ИсточникДанных) = Тип("ДеревоЗначений") Тогда 
...
Функция гПолучитьМеню(ИмяМеню, ДляУправляемойФормы = Ложь, ВВидеСписка = Ложь, Знач ВеткаДереваМеню = Неопределено) Экспорт
	...
	ИначеЕсли ИмяМеню = "Сервис" Тогда
		...
		МенюСервис = ВеткаДереваМеню.Строки.Добавить();
	...
	ИначеЕсли ИмяМеню = "Запросы" Тогда
		...
		Если Не ДляУправляемойФормы Тогда 
			...
			МенюЗапросы          = ВеткаДереваМеню.Строки.Добавить();
	...
	ИначеЕсли ИмяМеню = "Информация" Тогда
		...
		МенюИнформация          = ВеткаДереваМеню.Строки.Добавить();
	...
	ИначеЕсли ИмяМеню = "Дополнительно" Тогда
		...
		МенюДополнительно          = ВеткаДереваМеню.Строки.Добавить();
	...
	ИначеЕсли ИмяМеню = "СформироватьКод1С" Тогда

		КореньМеню          = ВеткаДереваМеню.Строки.Добавить();
		КореньМеню.Название = "Сформировать код 1С по запросу";
		КореньМеню.Имя      = "СформироватьКод1СПоЗапросу";
		КореньМеню.Картинка = ПолучитьВнешнююКартинку("create_from_query");
	
		НоваяСтрока          = КореньМеню.Строки.Добавить();
		НоваяСтрока.Название = "Сформировать код 1С по запросу (с выборкой)";
		НоваяСтрока.Имя      = "СформироватьКод1СПоЗапросуСВыборкой";
		НоваяСтрока.Действие = "КнопкаМенюСформироватьКод1СПоЗапросуСВыборкой";
		...
		НоваяСтрока          = КореньМеню.Строки.Добавить();
		НоваяСтрока.Название = "Сформировать код 1С по запросу (с таблицей значений)";
		НоваяСтрока.Имя      = "СформироватьКод1СПоЗапросуСТаблицейЗначений";
		НоваяСтрока.Действие = "КнопкаМенюСформироватьКод1СПоЗапросуСТаблицейЗначений";
		...
		НоваяСтрока          = КореньМеню.Строки.Добавить();
		НоваяСтрока.Название = "Сформировать код 1С по запросу (с обработкой результата)";
		НоваяСтрока.Имя      = "СформироватьКод1СПоЗапросуСОбработкойРезультата";
		НоваяСтрока.Действие = "КнопкаМенюСформироватьКод1СПоЗапросуСОбработкойРезультата";
		...
		КореньМеню.Имя      = "КнопкаМенюКодЗапросПолучитьЗапрос";
...
// Параметры:
//		ИсточникДанных          - {ТаблицаЗначений|ДеревоЗначений|СтрокаДереваЗначений} - источник данных для формирования печатной формы
...
Процедура ПечатьКоллекции(ИсточникДанных, ТабДок, ДополнительныеПараметры)
	...
	Для Каждого Выборка Из лДанныеКоллекция Цикл
		...
		Для ТекущееПоле = 0 По ДополнительныеПараметры.КоличествоКолонок - 1 Цикл
			Область = ИсходнаяСтрока.Область(1, ТекущееПоле + 1);
			Область.Текст = ЗначениеКВыводуНаПечать(Выборка[ТекущееПоле]);
