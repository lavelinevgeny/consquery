...
Функция СохранитьОбъектВФайл(СериализуемыйОбъект, ИмяФайла = "") Экспорт
	...
	ПутьКФайлу = лФайл.Путь + гСтроковыеКонтанты("ПрефиксФайлаССериализованнымОбъектом") + ?(ЗначениеЗаполнено(ИмяФайла), ИмяФайла, лФайл.Имя);
...
Функция гСкопироватьТекстВБуфер(Текст, Форма = Неопределено) Экспорт 
	...
	Если Форма = Неопределено Тогда 
		лhtmlfile = Новый COMОбъект("htmlfile"); лСтрокаБуфера = лhtmlfile.parentWindow.clipboardData.SetData("Text", Текст); 
	Иначе
		лНазваниеЭлемента = "ПолеHTMLДокумента321432458767_"; Форма.ЭлементыФормы.Добавить(Тип("ПолеHTMLДокумента"), лНазваниеЭлемента, Ложь); лОкно = Форма.ЭлементыФормы[лНазваниеЭлемента].Документ.parentWindow; 
...
Функция гПолучитьМеню(ИмяМеню, ДляУправляемойФормы, ВВидеСписка) Экспорт
	...
	ИначеЕсли ИмяМеню = "ПодменюВременныеТаблицы" Тогда
		...
		КореньМеню.Картинка = ПолучитьВнешнююКартинку("ПодменюВременныеТаблицы");
		...
		//НоваяСтрока.Картинка = ПолучитьВнешнююКартинку("create_from_query");
...
Функция гИзвлечьКартинкиОбработкиНаСервере() Экспорт
	...
	//лТекущийБезопасныйРежим = БезопасныйРежим();
	//                                                                  
	//Если лТекущийБезопасныйРежим <> Ложь Тогда 
	//	УстановитьОтключениеБезопасногоРежима(Истина);
	//КонецЕсли;
