...
Функция ДобавитьТипВКоллекциюСписокТиповПоЗначению(СписокТипов, Значение, СтруктураКЭШ = Неопределено) Экспорт
	
	ЗначениеТипа 		= XMLТип(Значение).ИмяТипа;
	ПредставлениеТипа 	= Строка(Значение);
	
	ИмяМетаданногоДляТипа = ПолучитьИмяБазовогоТипаПоТипуОбъекта(Значение, СтруктураКЭШ);
	КартинкаДляТипа       = ПолучитьКартинкуПоМетаданному(ИмяМетаданногоДляТипа, СтруктураКЭШ);
	
	Возврат СписокТипов.Добавить(ЗначениеТипа, ПредставлениеТипа,, КартинкаДляТипа);
...
Функция гСформироватьСписокТипов(СтруктураКЭШ = Неопределено, СписокДобавленныхТипов = Неопределено) Экспорт
	...
	Для каждого Стр Из НеПримитивныеТипы Цикл
		ДобавитьТипВКоллекциюСписокТиповПоЗначению(СписокТипов, Стр.Значение, СтруктураКЭШ);
	КонецЦикла;
