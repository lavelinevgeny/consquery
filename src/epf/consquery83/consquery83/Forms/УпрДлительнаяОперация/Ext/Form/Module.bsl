...
Функция ПроверитьЗаданиеВыполнено()
	...
	ПрогрессЗадания = ПолучитьИзВременногоХранилища(ИдентификаторЗадания);
	
	Если ЗначениеЗаполнено(ПрогрессЗадания) Тогда 
		Возврат Новый Структура("Статус, Прогресс", ПрогрессЗадания.Статус, ПрогрессЗадания);
	Иначе
		Возврат Новый Структура("Статус", "Ошибка")
	КонецЕсли;
