...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	лКореньМеню    = ЗаполнитьМенюПоДереву(КоманднаяПанель, ОбъектОбработки().гПолучитьМеню("ПредустановленныйКод", Истина, Ложь));
	...
	Если Не мВыполняетсяЗакрытие Тогда 
		Если Модифицированность Тогда 
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьИзмененияЗавершение", Вычислить(ЭтотОбъектФорма), 
				ПолучитьСтруктуруПараметровДляЗакрытияФормы(ВернутьЗначениеССервера("ОбъектОбработки().гРежимыЗакрытияФормыВводаПроизвольногоКода().Сохранить"), Истина)), 
				ТекстВопроса, 
...
Процедура ПредустановленныйКодВставитьЦикл(Команда)
	ПодставитьПредустановленныйКод(ТипыПредустановленногоКода().ВставитьЦикл);
КонецПроцедуры // ПредустановленныйКодВставитьЦикл

&НаКлиенте
Процедура ПредустановленныйКодРедактироватьРегистрСведений(Команда)
	ПодставитьПредустановленныйКод(ТипыПредустановленногоКода().РедактироватьРегистрСведений);
...
Процедура ПредустановленныйКодРедактироватьДокументы(Команда)
	ПодставитьПредустановленныйКод(ТипыПредустановленногоКода().РедактироватьДокументы);
КонецПроцедуры // ПредустановленныйКодРедактироватьДокументы

&НаКлиенте
Процедура ПредустановленныйКодРедактироватьСправочники(Команда)
	ПодставитьПредустановленныйКод(ТипыПредустановленногоКода().РедактироватьСправочники);
...
Процедура ПодставитьПредустановленныйКод(ВидПредустановленногоКода)
	...
	Если ВидПредустановленногоКода = ТипыПредустановленногоКода().ВставитьЦикл Тогда 
	...
	ИначеЕсли ВидПредустановленногоКода = ТипыПредустановленногоКода().РедактироватьРегистрСведений Тогда 
	...
	ИначеЕсли ВидПредустановленногоКода = ТипыПредустановленногоКода().РедактироватьДокументы Тогда 
		лЗначениеПоУмолчанию       = Неопределено;
	...
	ИначеЕсли ВидПредустановленногоКода = ТипыПредустановленногоКода().РедактироватьСправочники Тогда 
		лЗначениеПоУмолчанию       = Неопределено;
...
Процедура ОК(Команда)
	ЗакрытьФормуРедактирования(ПолучитьСтруктуруПараметровДляЗакрытияФормы(ВернутьЗначениеССервера("ОбъектОбработки().гРежимыЗакрытияФормыВводаПроизвольногоКода().Сохранить"), Истина));
КонецПроцедуры
...
Функция ПолучитьСписокОбъектовМетаданныхИзТекстаНаСервере(ИсходныйТекст, ФильтрМетаданных, ЗначениеПоУмолчанию)
	Возврат ОбъектОбработки().гПолучитьСписокОбъектовМетаданныхИзТекста(ИсходныйТекст, ФильтрМетаданных, ЗначениеПоУмолчанию, СтруктураКЭШ);
КонецФункции

&НаСервере
Функция ПолучитьПредустановленныйКодВставитьЦиклНаСервере()
	Возврат ОбъектОбработки().гПолучитьПредустановленныйКодВставитьЦикл(ПолучитьИзВременногоХранилища(АдресРезультата).Колонки);
...
Функция ПредустановленныйКодРедактироватьРегистрСведенийНаСервере(ПолноеИмяОбъектаМетаданных)
	Возврат ОбъектОбработки().гПолучитьПредустановленныйКодРедактироватьРегистрСведений(ПолноеИмяОбъектаМетаданных, ТекстТекущегоЗапроса);
...
Функция ПредустановленныйКодРедактироватьДокументНаСервере(ПолноеИмяОбъектаМетаданных)
	Возврат ОбъектОбработки().гПолучитьПредустановленныйКодРедактироватьДокумент(ПолноеИмяОбъектаМетаданных, ТекстТекущегоЗапроса);
...
Функция ПредустановленныйКодРедактироватьСправочникНаСервере(ПолноеИмяОбъектаМетаданных)
	Возврат ОбъектОбработки().гПолучитьПредустановленныйКодРедактироватьСправочник(ПолноеИмяОбъектаМетаданных, ТекстТекущегоЗапроса);
...
&НаСервереБезКонтекста
Функция ТипыПредустановленногоКода()
	Возврат ОбъектОбработки().гТипыПредустановленногоКода()
КонецФункции
...
&НаКлиенте
Процедура ЗакрытьФормуРедактирования(ПараметрыЗакрытия = Неопределено)
...
Процедура ОбработкаДействийКомандыВыполнить(ВыполнитьНаСервере)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СохранитьИзмененияЗавершение", Вычислить(ЭтотОбъектФорма), 
		ПолучитьСтруктуруПараметровДляЗакрытияФормы(ВернутьЗначениеССервера("ОбъектОбработки().гРежимыЗакрытияФормыВводаПроизвольногоКода().Выполнить"), ВыполнитьНаСервере)),
		ТекстВопроса, 
