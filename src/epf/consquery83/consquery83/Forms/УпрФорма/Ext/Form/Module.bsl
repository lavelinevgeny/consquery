...
Процедура ПараметрыТекущегоЗапросаТипЗавершениеВыбора(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда 
		Если РезультатЗакрытия.ВыбранныеТипы.Количество() > 0 Тогда
			...
			ПараметрТекущегоЗапросаОбновитьПоОписаниюТипов(СтрокаСПараметром, ОписаниеТиповСтрокойПоТипуСтрокой(ВыбранныйТип.ТипСтрокой));
...
Функция СписокЗначенийВТаблицуЗначений(ИсходнаяКоллекция, ИмяПоля = "Поле")
	...
	Для Каждого ЭлементКоллекции Из ИсходныйМассив Цикл 
...
&НаСервереБезКонтекста
Функция ДетальнаяИнформацияОТипеСоЗначениемНаСервере(ЗначениеВСтроке, УникальныйИдентификатор, АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка)
	...
	Иначе
		...
		Если Тип = Тип("Строка") Тогда 
			Если ЭтоПроизвольныйКод(Значение) Тогда 
...
КонецФункции // ДетальнаяИнформацияОТипеСоЗначениемНаСервере()

&НаСервереБезКонтекста
Функция ДетальнаяИнформацияОТипеСоЗначениемПоТипуНаСервере(ТипСтрокой, УникальныйИдентификатор, АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка)
...
КонецФункции // ДетальнаяИнформацияОТипеСоЗначениемПоТипуНаСервере()
...
Функция ПараметрыЗапросовНайти(Отбор, АдресОбщейТаблицыСПараметрамиЗапросов, АдресТаблицыСИнформациейПоТипам, УникальныйИдентификатор, Ошибки, АдресСеансовыхДанных)
	...
	Для Каждого ПараметрЗапроса Из ОтборанныеПараметры Цикл 
		...
		Если ПустаяСтрока(СтрокаСПараметром.Тип) Тогда 
			...
			ДетальныеДанныеПараметра = ДетальнаяИнформацияОТипеСоЗначениемНаСервере(СтрокаСПараметром.Значение, УникальныйИдентификатор, 
...
&НаСервереБезКонтекста
Функция КонвертироватьЗначениеПоОписаниюТипов(ИсходноеЗначениеСтрокой, ОписаниеНовогоТипаСтрокой, КонвертированноеЗначениеСтрокой, ТипКонвертированногоЗначенияПараметраСтрокой)
	...
	Если ОписаниеНовогоТипа.Типы().Количество() = 0  // Не удалось определить тип параметр в запросе, например, сам тип является полем запроса
		...
		ТипКонвертированногоЗначенияПараметраСтрокой = ЗначениеВСтроку(ТекущийТип);
		Возврат Ложь;
	...
	ТипКонвертированногоЗначенияПараметраСтрокой = ЗначениеВСтроку(НовыйТип);
...
&НаКлиенте
Функция ПараметрТекущегоЗапросаОбновитьПоОписаниюТипов(ПараметрТекущегоЗапроса, ОписаниеТипаСтрокой)
	
	Перем Ошибка, КонвертированноеЗначениеПараметраСтрокой, ТипКонвертированногоЗначенияПараметраСтрокой;
	
	ПараметрОбновлен = Ложь;
	
	Если КонвертироватьЗначениеПоОписаниюТипов(ПараметрТекущегоЗапроса.Значение, ОписаниеТипаСтрокой, КонвертированноеЗначениеПараметраСтрокой, ТипКонвертированногоЗначенияПараметраСтрокой) Тогда 
		
		ПараметрОбновлен = Истина;
		ПараметрТекущегоЗапроса.Значение = КонвертированноеЗначениеПараметраСтрокой;
		
		ДетальныеДанныеПараметра = ДетальнаяИнформацияОТипеСоЗначениемНаСервере(КонвертированноеЗначениеПараметраСтрокой, УникальныйИдентификатор, АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка);
...
Функция ПараметрТекущегоЗапросаДобавитьПоИмениИЗначению(Имя, Значение)
	...
	ДетальныеДанныеПараметра = ДетальнаяИнформацияОТипеСоЗначениемНаСервере(Значение, УникальныйИдентификатор, 
		АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка);
...
Функция ПараметрТекущегоЗапросаДобавитьПоИмениИТипу(Имя, ТипСтрокой)
	...
	Иначе
		ДетальныеДанныеПараметра = ДетальнаяИнформацияОТипеСоЗначениемПоТипуНаСервере(ТипСтрокой, УникальныйИдентификатор, АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка);
		Если ДетальныеДанныеПараметра = Неопределено Тогда 
			ВызватьИсключение Ошибка;	
		КонецЕсли;
	КонецЕсли;
...
Процедура ПараметрТекущегоЗапросаОбновитьПоИмениИЗначению(ПараметрТекущегоЗапроса, Имя, ЗначениеСтрокой)
	...
	ДетальныеДанныеПараметра = ДетальнаяИнформацияОТипеСоЗначениемНаСервере(ЗначениеСтрокой, УникальныйИдентификатор, 
		АдресТаблицыСИнформациейПоТипам, АдресСеансовыхДанных, Ошибка);
...
Процедура НЕИСПОЛЬЗУЕТСЯ_УстановитьУсловноеОформление()
	...
	//УстановитьЗапретПолейДляРедактирования("ПараметрыТекущегоЗапроса.Тип", ИдентификаторыТиповСоСпециальнымОтображением(), БлокируемыеПоля); 	
...
Процедура РезультатЗапросаЗагрузитьИзТаблицыЗначений(ЗагружаемаяКоллекцияВСтроке, АдресРезультата, ЗаменитьТекущуюТаблицу, ВариантКоллекции, ДобавлятьКолонки = Ложь, ДанныеКолонокНовойКоллекции = Неопределено)
	...
	Иначе	
		...
		КоличествоЗагружаемыхКолонок = Мин(КоличествоКолонокВЗагружаемойТЗ, НоваяТаблицаЗначений.Колонки.Количество());
		
		Для Каждого ТекущаяСтрока из ЗагружаемаяТЗ Цикл 
...
Функция СформироватьТаблицуЗначенийИзТекстаНаСервере(ИсходныйТекст, ИменаКолонокНовойКоллекции, ОпределятьТип, ПропуститьПервуюСтроку)
	...
	//ДлительностьЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЗамера;
	//Сообщить(ДлительностьЗамера);
