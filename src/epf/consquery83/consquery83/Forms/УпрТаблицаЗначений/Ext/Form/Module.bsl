...
Процедура НастройкиТаблицыЗначенийПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ИмяКолонки = ПолучитьСледующееУникальноеИмя(ИмяКолонкиПоУмолчанию, НастройкиТаблицыЗначений, "Имя");
...
Процедура НастройкиТаблицыЗначенийНаименованиеКолонкиОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	НовоеИмя = ПолучитьСледующееУникальноеИмя(СокрЛП(Текст), НастройкиТаблицыЗначений, "Имя");
...
Функция ПолучитьСледующееУникальноеИмя(Знач ИсходноеИмя, Коллекция, ИмяПоля = Неопределено) Экспорт
...
Функция НайтиЗначениеБезУчетаРегистра(Значение, Знач Коллекция, Поле = Неопределено, МетодПоиска = Неопределено, МножественныйВыбор = Ложь) Экспорт
	...
	Если МетодПоиска = МетодыПоискаВСтроке().Вконце Тогда 
	...
	Для Каждого СтрокаКоллекции Из Коллекция Цикл
		...
		Если МетодПоиска = Неопределено ИЛИ МетодПоиска = МетодыПоискаВСтроке().Точно Тогда 
		...
		Иначе
			...
			Если лРезультатПоиска = лКорректныйРезультатПоиска ИЛИ МетодПоиска = МетодыПоискаВСтроке().Посередине И лРезультатПоиска > 0 Тогда 
				Если Не МножественныйВыбор Тогда 
					Возврат СтрокаКоллекции
				КонецЕсли;
				Результат.Добавить(СтрокаКоллекции);
