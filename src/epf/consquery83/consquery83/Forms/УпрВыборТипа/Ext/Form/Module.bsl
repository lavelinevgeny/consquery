...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("АдресТаблицыСИнформациейПоТипам", АдресТаблицыСИнформациейПоТипам);
...
Функция РезультатВыбораНаСервере()
	...
	Для Каждого ТекВыбранныйТип Из ВыбранныеТипы Цикл 
		...
		СтруктураСДаннымиТипа.Вставить("ТипСтрокой", ДанныеВыбранногоТипа.ТипСтрокой);
		РезультатВыбора.Добавить(СтруктураСДаннымиТипа);
	КонецЦикла;

	Результат = Новый Структура;
	Результат.Вставить("ВыбранныеТипы"        , РезультатВыбора);
	Результат.Вставить("АдресТаблицыСИнформациейПоТипам", АдресТаблицыСИнформациейПоТипам);
...
Процедура ОбновитьСписокТипов()
	...
	Если Не ЗначениеЗаполнено(АдресТаблицыСИнформациейПоТипам) Тогда 
		...
		Если Не ОбъектОбработки().СформироватьТаблицуСТипами(АдресТаблицыСИнформациейПоТипам, УникальныйИдентификатор, Ошибки, АдресСеансовыхДанных) Тогда 
			СообщитьПользователю(СтрСоединить(Ошибки, Символы.ПС));
		КонецЕсли;
	КонецЕсли;

	ТаблицаСТипамиИсходная = ПолучитьИзВременногоХранилища(АдресТаблицыСИнформациейПоТипам);
...
Процедура ВыбратьИЗакрыть()
	...
	Если Не НастройкиФормы.МножественныйВыбор Тогда 
		...
		Если ТекущиеДанные <> Неопределено И Не ПустаяСтрока(ТекущиеДанные.ИмяДляКонструктора) Тогда 
			ТекущиеДанные.Пометка = 1;
			ОбработатьПометкуПоТекущимДанным(ТекущиеДанные);			
		КонецЕсли;
	КонецЕсли;
