...
Процедура ПодключенияКОблакуДействие()
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		лФормаДиалога = ВнешниеОбработки.ПолучитьФорму(Обработка.ИспользуемоеИмяФайла,"ФормаПодключениеКОблаку",мЭтотОбъектФорма);
	#Иначе
...
Функция ЗагрузитьЗапросыИзФайлаСтараяВерсия(имяФайла, ТекстОшибки) Экспорт
	...
	Если Не Ошибка Тогда 
		Попытка
			ПараметрыВерсииОбработки = Новый Структура("ОбновлятьИД, ЭтоСтарыйРелиз, ЕстьВариантыКода", Истина, Истина,
				ДеревоЗапросов.Колонки.Найти("ВариантыКода") <> Неопределено);
			СкопироватьДеревоЗапросовРекурсивно(ДанныеИзФайла, ДеревоЗапросов, ПараметрыВерсииОбработки);
		Исключение
			Ошибка = Истина;
			ТекстОшибки = ОписаниеОшибки();	
		КонецПопытки; 
...
Процедура ПоказатьДиалогВыбораФайла() // #Рефакторинг -> Унифицировать с аналогичной процедурой из управляемой формы
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		Если лДиалог.Выбрать() Тогда
			ДиалогВыбораФайлаЗавершение(лДиалог.ВыбранныеФайлы, лДополнительныеПараметры);
		КонецЕсли;
	#Иначе
