...
Перем гСеансовыеДанные Экспорт;           // структура для хранения данных в рамках текущего сеанса (для обычных форм)
										  
#КонецОбласти

Функция ВерсияОбработки()
	// 1.0.0 — первый стабильный релиз.
	// 1.1.0 — добавлен новый функционал, обратно совместим.
	// 1.1.1 — исправлена ошибка.
	// 2.0.0 — существенные изменения, несовместимые с предыдущими версиями.
	Возврат "4.0.1";
	...
	#Иначе
		Если Запрос.МенеджерВременныхТаблиц <> Неопределено Тогда 
			Для Каждого ВременнаяТаблица Из Запрос.МенеджерВременныхТаблиц.Таблицы Цикл
				...
				//гПолучитьУникальноеИмяВКоллекции(ИсходноеИмя, СписокПараметров, "Имя")
...
//  	Параметры - массив структур - ["Имя", "Значение"]: имя параметра, значение параметра
Функция гПолучитьЗапросИзТекста(Код1С, ПростаяОбработкаТекста) Экспорт
	
	Перем ПараметрыЗапроса; 
	Перем ТекстовыйБлокСПоследнимЗапросом;

	ТекстовыйБлокСПоследнимЗапросом = Код1С;
	
	// Парсим из кода 1с установку параметров
	ПараметрыЗапроса = Новый Массив;
	Если Не ПростаяОбработкаТекста Тогда
		СложнаяОбработкаТекстаЗапроса(ПараметрыЗапроса, ТекстовыйБлокСПоследнимЗапросом, Код1С);
	КонецЕсли;
	
	// очищаем код 1с для получения текста с запросом
	ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, "|", "");
	ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, """;", "");
	ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, Символ(34) + Символ(34), "_БывшаяДвойнаяКавычка_"); 
	ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, Символ(34), ""); 
	ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, "_БывшаяДвойнаяКавычка_", Символ(34)); 
	
	Возврат Новый Структура("Текст, Параметры", ТекстовыйБлокСПоследнимЗапросом, ПараметрыЗапроса);
...
Функция гВременнаяТаблицаВТЗ(ИмяВременнойТаблицы, МенеджерВременныхТаблиц) Экспорт
	...
	Если МенеджерВременныхТаблиц <> Неопределено Тогда
		...
		Исключение
			//Сообщить("Ошибка получения временной таблицы [" + ИмяВременнойТаблицы + "] - " + ОписаниеОшибки());
...
Функция ДетальнаяИнформацияПоТипам(ДоступныеТипы, Ошибки, СеансовыеДанные = Неопределено)
	...
	Результат.Колонки.Добавить("ТипСтрокой", ОписаниеТипаСтроки);
	...
	Для Каждого ТипДанных Из ДоступныеТипы Цикл 
		...
		НоваяСтрока.ТипСтрокой = ЗначениеВСтроку(ТипДанных);
		
		Если ТипДанных = гТипПараметра_ПроизвольныйКод() Тогда 
			НоваяСтрока.ИмяДляКонструктора = гТипПараметра_ПроизвольныйКод();
			НоваяСтрока.Вид = гВидыПараметров().НеОтображаемыйВФорме;
		...
		//  НоваяСтрока.Вид = гВидыПараметров().Структура;
		// #ДЛЯ РЕДАКТИРОВАНИЯ# - Здесь необходимо прописать недостающие типы данных вручную
		ИначеЕсли ТипЗнч(ТипДанных) = Тип("Тип") Тогда 
			...
			Если ТипXML.URIПространстваИмен = URIПространстваИменТипов().core Тогда 
				НоваяСтрока.Вид = гВидыПараметров().НеОтображаемыйВФорме;
			Иначе
				НоваяСтрока.Вид = гВидыПараметров().ОтображаемыйВФорме;
			КонецЕсли;			
		КонецЕсли;
...
Функция СформироватьТаблицуСТипами(АдресТаблицыСИнформациейПоТипам, УникальныйИдентификаторФормы, Ошибки, СеансовыеДанные = Неопределено) Экспорт
	...
	ДоступныеТипы.Добавить(гТипПараметра_ПроизвольныйКод());
	
	ТаблицаСТипами = ДетальнаяИнформацияПоТипам(ДоступныеТипы, Ошибки, СеансовыеДанные);
	
	АдресТаблицыСИнформациейПоТипам = ПоместитьВоВременноеХранилище(ТаблицаСТипами, УникальныйИдентификаторФормы);
		
	Возврат Не (ЗначениеЗаполнено(Ошибки) И Ошибки.Количество() > 0);
	
КонецФункции // СформироватьТаблицуСТипами()

Функция гВидыПараметров() Экспорт
...
Функция гТипПараметра_ПроизвольныйКод() Экспорт
	Возврат "ПроизвольныйКод";
КонецФункции

Функция гЭтоПроизвольныйКод(СтроковоеЗначение) Экспорт
	
	Если Лев(СтроковоеЗначение, СтрДлина(ПрефиксЗначенияПараметра_ПроизвольныйКод())) = ПрефиксЗначенияПараметра_ПроизвольныйКод() Тогда 
		Возврат Истина
	Иначе
		Возврат Ложь	
	КонецЕсли;
...
Функция гДобавитьВЗначениеПрефиксПроизвольногоКода(ЗначениеПараметра) Экспорт
	
	Результат = СтрШаблон("%1%2", ПрефиксЗначенияПараметра_ПроизвольныйКод(), ЗначениеПараметра);
	
	Возврат Результат;
	
КонецФункции

Функция ПрефиксЗначенияПараметра_ПроизвольныйКод()
	Перем Префикс;
	Префикс = СтрШаблон("#%1#:", гТипПараметра_ПроизвольныйКод());
	Возврат Префикс;
КонецФункции

Функция УдалитьПрефиксСтроки(Префикс, Строка)
	
	ДлинаПрефикса = СтрДлина(Префикс);
	Если ПустаяСтрока(Префикс) Тогда 
		Результат = Строка;
	Иначе
		Результат = Сред(Строка, ДлинаПрефикса + 1);
	КонецЕсли;
...
Функция гУдалитьИзЗначенияПрефиксПроизвольногоКода(ЗначениеПараметра) Экспорт
	
	Результат = УдалитьПрефиксСтроки(ПрефиксЗначенияПараметра_ПроизвольныйКод(), ЗначениеПараметра);
...
Функция КоллекциюМожноВыгрузитьВМассив(Коллекция)

	ПростыеКоллекции = Новый Массив;
	ПростыеКоллекции.Добавить(Тип("ТаблицаЗначений"));
	ПростыеКоллекции.Добавить(Тип("Массив"));
	ПростыеКоллекции.Добавить(Тип("СписокЗначений"));
	
	Возврат ( ПростыеКоллекции.Найти(ТипЗнч(Коллекция)) <> Неопределено );

КонецФункции

Функция ПреобразоватьКоллекциюВМассив(Коллекция, Поле = Неопределено)
	
	Если Не КоллекциюМожноВыгрузитьВМассив(Коллекция) Тогда 
		Возврат Неопределено
	КонецЕсли;
	...
	Если ТипКоллекции = Тип("ТаблицаЗначений") Тогда
		...
		Если Поле = Неопределено Тогда 
			ВызватьИсключение "Не указано поле для таблицы в процедуре гНайтиЗначениеБезУчетаРегистра";	
		КонецЕсли;
		
		Результат = Коллекция.ВыгрузитьКолонку(Поле);
	ИначеЕсли ТипКоллекции = Тип("Массив") Тогда
		Результат = Коллекция;
	ИначеЕсли ТипКоллекции = Тип("СписокЗначений") Тогда
		Результат = Коллекция.ВыгрузитьЗначения();
	Иначе
		ТекстОшибки = СтрШаблон("В функцию ПреобразоватьКоллекциюВМассив передана коллекция некорректного типа %1", ТипКоллекции);
...
Функция гПолучитьУникальноеИмяВКоллекции(ИсходноеИмя, Коллекция, ИмяПоля = Неопределено) Экспорт
	
	Перем УникальноеИмя;
	
	УникальноеИмя = ИсходноеИмя;
	
	ИндексУникальности = 1;	
	
	РезультатПоиска = Истина;
	Пока РезультатПоиска <> Неопределено Цикл
		
		Если КоллекциюМожноВыгрузитьВМассив(Коллекция) Тогда 
			РезультатПоиска = гНайтиЗначениеБезУчетаРегистра(УникальноеИмя, Коллекция, ИмяПоля);
		ИначеЕсли Не гНайтиЭлементыКоллекцииПоКлючуБезУчетаРегистра(УникальноеИмя, Коллекция, ИмяПоля, Истина).Количество() Тогда 
			РезультатПоиска = Неопределено;
		КонецЕсли;
		
		УникальноеИмя = СтрШаблон("%1_%2", ИсходноеИмя, Формат(ИндексУникальности, "ЧГ=0"));
		ИндексУникальности = ИндексУникальности + 1;
	КонецЦикла;
...
Функция гНайтиЗначениеБезУчетаРегистра(ИскомоеЗначение, Коллекция, Поле = Неопределено) Экспорт
	
	Перем РазмерКоллекции, Сч;
	
	Если ТипЗнч(ИскомоеЗначение) <> Тип("Строка") Тогда 
		ТекстОшибки = СтрШаблон("В функцию гНайтиЗначениеБезУчетаРегистра передано значение некорректного тип %1", ТипЗнч(ИскомоеЗначение));
		ВызватьИсключение ТекстОшибки;	
	КонецЕсли;
	...
	ДанныеКолонки = ПреобразоватьКоллекциюВМассив(Коллекция);
	
	Если ДанныеКолонки = Неопределено Тогда 
		ТекстОшибки = СтрШаблон("В функцию гНайтиЗначениеБезУчетаРегистра передана коллекция некорректного типа %1", ТипЗнч(Коллекция));
		ВызватьИсключение ТекстОшибки;	
	КонецЕсли;
	
	ТаблицаДляПоиска = Новый ТаблицаЗначений;
	ТаблицаДляПоиска.Колонки.Добавить("Ключ", Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(500)));
	
	РазмерКоллекции = ДанныеКолонки.Количество();
	Для Сч = 1 По РазмерКоллекции Цикл 
		ТаблицаДляПоиска.Добавить();
	КонецЦикла;
	ТаблицаДляПоиска.ЗагрузитьКолонку(ДанныеКолонки, "Ключ");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Выборка.Ключ КАК Ключ
	|ПОМЕСТИТЬ ТаблицаДляПоиска
	|ИЗ
	|	&ТаблицаДляПоиска КАК Выборка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ТаблицаДляПоиска.Ключ КАК Ключ
	|ИЗ
	|	ТаблицаДляПоиска КАК ТаблицаДляПоиска
	|ГДЕ
	|	ТаблицаДляПоиска.Ключ = &ИскомоеЗначение";
...
Функция гНайтиЭлементыКоллекцииПоКлючуБезУчетаРегистра(Ключ, Коллекция, ПолеКоллекции, ТочныйПоиск) Экспорт
	...
	Для Каждого ЭлементКоллекции Из Коллекция Цикл
		...
		ТекущееЗначение = ЭлементКоллекции[ПолеКоллекции];
		
		Если ТочныйПоиск Тогда 

			Если НРЕГ(ТекущееЗначение) = НРЕГ(Ключ) Тогда
				Результат.Добавить(ЭлементКоллекции);
			КонецЕсли;

		Иначе
			
			Если СтрНайти(НРЕГ(ТекущееЗначение), НРЕГ(Ключ)) > 0 Тогда 
				Результат.Добавить(ЭлементКоллекции);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла; 
...
КонецФункции // гНайтиЭлементыКоллекцииПоКлючуБезУчетаРегистра()
...
//
Функция гПреобразоватьВПравильноеНазвание(Знач ИсходнаяСтрока, ДополнительныеСимволы = "") Экспорт
	...
	ШаблонДоступныеСимволы = ЗаменительНекорректныхСимволов + "qwertyuiopasdfghjklzxcvbnmйцукенгшщзхъфывапролджэячсмитьбю" + ДополнительныеСимволы;
	ШаблонДоступныеНеНачальныеСимволы = "1234567890";
	
	ПреобразованнаяСтрока = ИсходнаяСтрока;
	ПреобразованнаяСтрока = СокрЛП(ПреобразованнаяСтрока);
	
	сч = 1;
	Если Не Найти(ШаблонДоступныеСимволы, НРег(Лев(ПреобразованнаяСтрока, 1))) Тогда 
		сч = 2;
		ПреобразованнаяСтрока = ЗаменительНекорректныхСимволов + ПреобразованнаяСтрока;
	КонецЕсли;
	
	ДоступныеСимволы = ШаблонДоступныеСимволы + ШаблонДоступныеНеНачальныеСимволы;
	Пока сч <= СтрДлина(ПреобразованнаяСтрока) Цикл

		ТекущийСимвол = Сред(НРег(ПреобразованнаяСтрока), сч, 1);

		Если Не Найти(ДоступныеСимволы, ТекущийСимвол) Тогда
			ПреобразованнаяСтрока = СтрЗаменить(ПреобразованнаяСтрока, ТекущийСимвол, ЗаменительНекорректныхСимволов);
		КонецЕсли;
...
Процедура гИнициализацироватьСеансовыеДанныеОФ(СеансовыеДанные) Экспорт

	СеансовыеДанные = Новый Структура();	
	СеансовыеДанные.Вставить("ИдентификаторСеанса", Новый УникальныйИдентификатор);
	СеансовыеДанные.Вставить("ПутьККартинкам"     , ИзвлечьКартинкиОбработкиНаСервереОФ(СеансовыеДанные));
	СеансовыеДанные.Вставить("ДоступныеТипыДанных", Метаданные().Реквизиты.ДоступныеТипыДанных.Тип.Типы());
	
КонецПроцедуры // гИнициализацироватьСеансовыеДанныеОФ()
...
Функция ЗначениеИзСеансовыхДанныхОФ(СеансовыеДанные, Имя) Экспорт
...
КонецФункции // ЗначениеИзСеансовыхДанныхОФ()
...
Функция ПолучитьHTTPСоединение(Сервер, НастройкиПрокси)
	...
	Попытка
		Если ЗначениеЗаполнено(НастройкиПрокси) И НастройкиПрокси.Использовать Тогда 
			...
			лСоединение = Новый HTTPСоединение(Сервер,,,, лПроксиСервер, ТаймаутПоУмолчанию());
		Иначе
			лСоединение = Новый HTTPСоединение(Сервер,,,,, ТаймаутПоУмолчанию());
		КонецЕсли;
		лСоединение = Новый HTTPСоединение(Сервер,,,,, ТаймаутПоУмолчанию());
	Исключение
...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ДобавитьКоманду(ТаблицаКоманд, "Консоль запросов", "consquery", "ВызовКлиентскогоМетода", Истина);
...
Функция ИдЭлементаОформленияСпецПоля()
	Возврат "ОформлениеСпецПоля";
КонецФункции

Процедура УдалитьЭлементОформленияСпецПоля(УсловноеОформление)
	
	Перем ЭлементаОформленияСпецПоля;
	
	ЭлементаОформленияСпецПоля = Неопределено;	
	Для Каждого ЭлементОформления Из УсловноеОформление.Элементы Цикл 
		Если ЭлементОформления.ИдентификаторПользовательскойНастройки = ИдЭлементаОформленияСпецПоля() Тогда
			ЭлементаОформленияСпецПоля = ЭлементОформления;
			Прервать;	
		КонецЕсли;
	КонецЦикла;
	
	Если ЭлементаОформленияСпецПоля <> Неопределено Тогда 
		УсловноеОформление.Элементы.Удалить(ЭлементОформления);
	КонецЕсли;
	
КонецПроцедуры // УдалитьЭлементОформленияСпецПоля()

Процедура ДобавитьЭлементОформленияСпецПоля(УсловноеОформление, лСпецПоля, ИмяРеквизитаФормыРезультатЗапроса)

	Если лСпецПоля.Количество() Тогда 
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ЭлементУсловногоОформления.ИдентификаторПользовательскойНастройки = ИдЭлементаОформленияСпецПоля();
...
КонецПроцедуры // ДобавитьЭлементОформленияСпецПоля()
...
Процедура гВывестиТаблицуНаФорму(Элементы, АдресРезультата, АдресСпискаСпецПолей, УсловноеОформление, СоздатьДерево = Ложь) Экспорт
	...
	ЭлементФормыТЗ.Отображение = ?(СоздатьДерево, ОтображениеТаблицы.Дерево, ОтображениеТаблицы.Список);
	...
	УдалитьЭлементОформленияСпецПоля(УсловноеОформление);

	ДобавитьЭлементОформленияСпецПоля(УсловноеОформление, лСпецПоля, ИмяРеквизитаФормыРезультатЗапроса);
	// --- добавляем оформление для спец полей
...
Функция ИзвлечьКартинкиОбработкиНаСервереОФ(СеансовыеДанные)

	лВременныйФайл = Новый Файл(ПолучитьИмяВременногоФайла());
	
	лПутьККартинке = лВременныйФайл.Путь + гПреобразоватьВПравильноеНазвание(гНазваниеОбработки() + "\" + ЗначениеИзСеансовыхДанныхОФ(СеансовыеДанные, "ИдентификаторСеанса"), "\");
...
КонецФункции // ИзвлечьКартинкиОбработкиНаСервереОФ()
...
Функция ЕстьКартинкавоВнешнейБиблиотеке(ИмяКартинки)
	...
	КартинкиБиблиотеки.Добавить(НРег(гТипПараметра_ПроизвольныйКод()));
...
#Область ИспользованиеРегулярныхВыражений

Функция ЗначениеРезультатаРегулярногоВыраженияПоИндексу(КоллекцияСРезультатом, Знач Индекс = Неопределено)
	
	Если Индекс = Неопределено Тогда 
		Индекс = КоллекцияСРезультатом.Count - 1;
	КонецЕсли;
	
	Результат = КоллекцияСРезультатом.Item(Индекс).Value;
	Результат = СокрЛП(Результат);
	
	Возврат Результат;
	
КонецФункции // ЗначениеРезультатаРегулярногоВыраженияПоИндексу()

Процедура СложнаяОбработкаТекстаЗапроса(ПараметрыЗапроса, ТекстовыйБлокСПоследнимЗапросом, Код1С)

	Перем ТекстовыеБлокиСЗапросами, ТекстовыеБлокиСУстановкойПараметров, ТекстовыйБлокСУстановкойПараметра;
	Перем ТекстВСкобках, ТекстМеждуСкобками, ИмяИЗначение, ИмяПараметра, ЗначениеПараметра; 
	Перем ШаблонИмяЗначение, ШаблонТекстВСкобках, ШаблонКод1ССЗапросом, ШаблонУстановкаПараметра; 
	
	Если Не гИнициализацияVBScript() Тогда 
		Возврат;
	КонецЕсли;
	
	// Строка вида: "Выбрать что-то "";"
	ШаблонКод1ССЗапросом = "(^|\s+|\t+|""|\|)[Вв][Ыы][Бб][Рр][Аа][Тт][Ьь][\s\S\t\n\r]*"";";
	
	// Строка вида: "УстановитьПараметр(значение);" 
	ШаблонУстановкаПараметра = "[Уу][Сс][Тт][Аа][Нн][Оо][Вв][Ии][Тт][Ьь][Пп][Аа][Рр][Аа][Мм][Ее][Тт][Рр]\(([^\n]*)\);";
	ШаблонТекстВСкобках = "\((.*)\)";
	ШаблонИмяЗначение = "([^,\s\t]+[^,]*)";
	...
	// \n	 - новая строка
	// \r	 - перевод каретки
	гRegExp.Pattern = ШаблонКод1ССЗапросом;
	ТекстовыеБлокиСЗапросами = гRegExp.Execute(Код1С);
			
	Если ТекстовыеБлокиСЗапросами.Count > 0 Тогда
		
		ТекстовыйБлокСПоследнимЗапросом = ЗначениеРезультатаРегулярногоВыраженияПоИндексу(ТекстовыеБлокиСЗапросами);
		ТекстовыйБлокСПоследнимЗапросом = СтрЗаменить(ТекстовыйБлокСПоследнимЗапросом, Символ(13) + Символ(10), Символы.ПС);

		// Заполнение параметров
		гRegExp.Pattern = ШаблонУстановкаПараметра;
		ТекстовыеБлокиСУстановкойПараметров = гRegExp.Execute(Код1С);
		
		Для Каждого ТекстовыйБлокСУстановкойПараметра Из ТекстовыеБлокиСУстановкойПараметров Цикл 
			
			гRegExp.Pattern = ШаблонТекстВСкобках;
			ТекстВСкобках = гRegExp.Execute(СокрЛП(ТекстовыйБлокСУстановкойПараметра.Value));
			Если ТекстВСкобках.Count > 0 Тогда
				
				ТекстМеждуСкобками = ЗначениеРезультатаРегулярногоВыраженияПоИндексу(ТекстВСкобках, 0);						
				ТекстМеждуСкобками = Сред(ТекстМеждуСкобками, 2, СтрДлина(ТекстМеждуСкобками) - 2); // убираем первую и последнюю скобки
				
				гRegExp.Pattern = ШаблонИмяЗначение;
				ИмяИЗначение = гRegExp.Execute(ТекстМеждуСкобками);
				Если ИмяИЗначение.Count = 2 Тогда
					ИмяПараметра = Вычислить(ЗначениеРезультатаРегулярногоВыраженияПоИндексу(ИмяИЗначение, 0));
					ЗначениеПараметра = Вычислить(ЗначениеРезультатаРегулярногоВыраженияПоИндексу(ИмяИЗначение, 1));
					ПараметрыЗапроса.Добавить(Новый Структура("Имя, Значение", ИмяПараметра, ЗначениеВСтроку(ЗначениеПараметра)));
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры // СложнаяОбработкаТекстаЗапроса()
